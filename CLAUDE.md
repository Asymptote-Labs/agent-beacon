...
- Session handoff via `beacon handoff list|export|resume` (`cli/beacon/internal/handoff`, `cli/beacon/cmd/handoff*.go`; docs in `docs/cli/handoff.mdx`) for Claude Code, Codex CLI, OpenCode and Cline. It reads each runtime's own session store read-only and never touches the network. `resume` reopens a session natively (`claude --resume`, `codex resume`, `opencode --session`, `cline --tui --id`) when it can, and otherwise writes a brief to `~/.beacon/endpoint/handoffs` (0700/0600) and starts a new session pointed at it. The brief is passed by absolute path, never inlined on a command line. Every Cline launch passes `--auto-approve false`, because Cline defaults to approving every tool call. Briefs run every event through `SanitizeEvent`, and quoted content goes in fences longer than any backtick run it contains, with inline fields reduced to one line, so quoted content cannot forge brief structure. A new session's first prompt ends with the `[beacon-handoff from=<harness> session=<id>]` marker (`pkg/asymptoteobserve/handoff.go`). The hook prompt paths and the four session-store mappers record a `session.handoff` event with `handoff.{source_harness,source_session_id}` when they see it; that is an observed claim, not a verified identity.
