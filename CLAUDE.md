...
- `beacon login` is the account-identity channel for interactive users and is required by the interactive `beacon endpoint install` wizard. It uses browser PKCE against beacon.sh, stores the scoped CLI token only in `~/.beacon/auth/session.json` (0600 under a 0700 directory), and is separate from endpoint enrollment: signing in alone never starts forwarding or sends telemetry. System/root/package/MDM/CI/piped-stdin/dry-run installs stay noninteractive and account-free. Keep account credentials out of endpoint config, hook environments, logs, and command output.
