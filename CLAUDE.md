...
- Prime Agent hook telemetry through a Beacon-managed local extension (`~/.prime/agent/extensions/beacon.ts`, or `.prime/agent/extensions/beacon.ts` at project level -- both keep the `agent` segment, unlike Pi). Prime Agent ships Pi's extension API under a rebranded config directory, so one extension source in `plugins/pi-beacon` is rendered for both, parameterized by a runtime name; do not fork a second copy. The envelope -- session lifecycle, prompts, operator `!` commands, assistant messages -- is mapped by the shared `cli/beacon-hooks/cmd/pi_family.go`; only the tool surface is Prime Agent's own (`prime_event.go`), and that is the only part worth writing twice. Its agent has one tool, `ipython`, a cell of Python in a persistent kernel, so the cell source is recorded in `command.command` -- that is the runtime's whole execution surface, and leaving it out would mean no command-shaped rule ever matches this runtime. Coverage: session lifecycle, prompts, kernel cells with duration and output, streamed file edits (`details.diffs`, the only structured file signal the runtime produces), agent-to-agent messages as `agent.message.sent`, `session.compacted`, `agent.harness.refined` for durable self-modification, agent reasoning, and token usage/cost. The extension subscribes to nine event types and ignores streaming and provider internals. Approval decisions are not exposed and are not synthesized, matching the Pi and Cline decisions. No exit code is derived from the kernel's ok/error/aborted status.
