...
- A second optional, off-by-default seam for synchronous cloud detection (`cli/beacon-hooks/internal/mdr`), consulted only by the `prompt-submit` hook. When `BEACON_MDR_URL` is set, the hook POSTs the submitted prompt plus session/repository context to that endpoint with `BEACON_MDR_TOKEN` as a bearer token, and honors an `allow`/`steer`/`block` verdict: a steer becomes `hookSpecificOutput.additionalContext` plus a `systemMessage` banner, a block becomes the runtime's native prompt rejection, and either writes `policy.steered`/`policy.blocked` telemetry using the existing `PolicyInfo` fields. It is deliberately separate from the `policycontract` seam above rather than an extension of it: that contract is versioned and published, its rule is "anything that is not an explicit deny is an allow", and it models a *local executable* decider. The seam is inert when `BEACON_MDR_URL` is unset and fail-open on every error (dial failure, non-2xx, timeout, malformed output, unrecognized decision), so the open build still ships no enforcement of its own and requires no hosted account for normal hook execution.
