...
- `beacon login` is the optional account-identity channel for interactive users. It uses browser PKCE against beacon.sh, stores the scoped CLI token only in `~/.beacon/auth/session.json` (0600 under a 0700 directory), and is separate from endpoint enrollment: signing in alone never starts forwarding or sends telemetry. Keep account credentials out of endpoint config, hook environments, logs, and command output.
- Asymptote Managed forwarding (`cli/beacon/internal/endpoint/asymptote`, `beacon endpoint connect`, `beacon endpoint asymptote`) is opt-in and Vector-shaped: Beacon writes local JSONL, Vector ships new lines after browser approval, and the `bcn_device_*` key stays only in `<BaseDir>/asymptote/vector-secrets.json` via Vector's secret backend. Keep the ingest URL `https://`, the wire contract aligned with `beacon-ingest` (`/v1/ingest/{runtime,inventory}` and `/v1/ingest/health`), and do not add another endpoint network channel. The cloud-sandbox shuttle is the bounded exception: only with `BEACON_ORIGIN=cloud`, it can upload new runtime lines to GCS, S3, or the same Asymptote ingest contract because no Vector/service manager/browser/persistent disk exists there.
- `beacon endpoint install` has a bounded onboarding channel for open-source attribution and destination choice. It is interactive TTY only, once per machine, skipped for system/root/CI/piped-stdin/dry-run or `BEACON_ONBOARDING=0`, and failures never fail install. It sends install context only, never prompts, events, file content, or command output. Implementation lives in `cli/beacon/internal/onboarding` and `cli/beacon/cmd/endpoint_onboarding.go`; keep `docs/cli/endpoint-onboarding.mdx` in sync with payload changes and do not copy the field list into the README.
...
- Claude Code and Codex CLI use local OpenTelemetry settings; Claude/Codex hooks add runtime-native context where configured. `beacon endpoint codex sync` is the offline poll backfill from `~/.codex/sessions`; it never reads `~/.codex/auth.json` and cannot approve, deny, or delay tool calls.
...
- DeepSeek Harness (`dsh`) is installed through two user-level files: `$DSH_HOME/beacon-endpoint-hooks.json` and a row in `$DSH_HOME/cordis.patch.yml`; there is no project scope. It rides the Claude-shaped envelope, but `run_code` remains `tool.invoked` because nested commands fire their own hooks.
