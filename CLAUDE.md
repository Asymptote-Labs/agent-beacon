...
- Grok Bot (xAI) telemetry from Cursor's Enterprise server-side OpenTelemetry Export, mapped by the `beaconjson` exporter (`collector-builder/exporter/beaconjsonexporter/internal/beaconevent/cursor_grokbot.go`). Grok Bot runs on Cursor-hosted cloud computers and exposes no hook, plugin, session store, or local OTLP surface, so this is a cloud-agent path: the export (OTLP/HTTP protobuf, `cursor.surface=grok_bot` on the resource, event name in the log body) reaches a collector the customer runs, and the endpoint agent only detects the desktop app (`~/.grokbot`, capability `cloud_export`, informational in doctor). Grok Bot and Grok Build are different products: `grok_bot` is a closed-set case in `NormalizeHarnessName` that must never claim `grok` (Grok Build's harness) or a Grok model id. Mapping follows Cursor's wire reference exactly: shell commands to `command.executed`, or `approval.denied` when Cursor's own `shell.allowed` is false (observed, not synthesized), MCP calls to `mcp.tool_invoked`/`tool.failed`, browser navigation and computer use to `tool.invoked`, `api_request` token counts into `gen_ai.usage`; `origin` is `cloud` except for `shell.target=user_machine`. Prompts, tool arguments, file activity, and approval requests are absent because the export carries none; do not invent them. The desktop app's local-execution helper log under `~/.grokbot` is undocumented and deliberately not read.
