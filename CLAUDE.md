...
Install both goose paths during manual testing (the two are separate commands on purpose: hooks
see prompts, tools, commands and file edits; OTLP carries the token usage, model and reasoning goose
puts on no hook):

```bash
cd cli/beacon
go run . endpoint hooks install --harness goose     # plugin hooks in ~/.agents/plugins/beacon-endpoint
go run . endpoint install --harness goose           # otel_exporter_otlp_endpoint in goose's config.yaml
go run . endpoint hooks status --harness goose
```
...
- `bun run check && bun test` in `plugins/opencode-beacon`, `plugins/cline-beacon`, `plugins/pi-beacon`, and `plugins/omp-beacon`.
