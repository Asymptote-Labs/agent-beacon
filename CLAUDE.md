...
- `plugins`: TypeScript sources for the managed runtime plugins/extensions (`opencode-beacon`, `cline-beacon`, `pi-beacon`, `omp-beacon`, `openclaw-beacon`), each mirrored into an embedded copy under `cli/beacon/internal/endpoint/hooks/assets/`.
...
- Runtimes that ride the shared hook and OTLP paths without a contract worth writing down here: Antigravity CLI, Devin CLI, Devin Desktop, Factory Droid, Gemini CLI, GitHub Copilot CLI, Grok Build, Hermes Agent, OpenCode (a Beacon-managed plugin, `plugins/opencode-beacon`), Qwen Code, and VS Code. The bullets above are the runtimes with a reverse-engineered or otherwise surprising contract, not the full supported set — `harness.DiscoverAll` in `cli/beacon/internal/endpoint/harness` and the install switch in `cli/beacon/cmd/endpoint_hooks.go` are the authoritative lists. Prime Agent has a reserved harness name and a `plugin` collection method but no collection path yet.
