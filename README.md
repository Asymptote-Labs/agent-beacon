...
| [Grok Bot](https://docs.asymptotelabs.ai/runtimes/grok-bot) | Cursor Enterprise OpenTelemetry Export received by a customer-run Beacon collector; the desktop app is only detected locally | Shell commands with the shell-policy decision, MCP tool calls, browser navigation, computer-use sessions, model requests with token usage, and reported cost. No prompt text, tool arguments, or file activity, because Cursor's export carries none |
