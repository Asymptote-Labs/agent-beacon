...
description: "Forward Beacon endpoint events to Beacon's managed ingest service and see them on the Beacon dashboard."
---

## Forwarding overview

Beacon Managed is the forwarding path that lands Beacon endpoint events on the Beacon dashboard without any customer-run infrastructure. Beacon stays the local JSONL producer. A Vector forwarder on the endpoint tails the [runtime JSONL log](/concepts/core-concepts#runtime-jsonl-log) and the inventory log and POSTs gzip-compressed NDJSON batches over HTTPS to Beacon's ingest service, authenticated with a key issued to that one device.

This is the only Beacon destination that sends telemetry to a Beacon-run endpoint. It is preselected during interactive onboarding, and confirming it mints a device key and starts the forwarder in the same command, once the local install has succeeded. Signing in alone forwards nothing. Choosing Local explicitly opts out. Revoking the device from the dashboard's Beacon Endpoints page stops ingestion within about a minute.
...
For runtime telemetry, Standard mode forwards every line written after the
forwarder starts exactly as Beacon wrote it locally; pre-connect runtime history is
not sent. Inventory is read from the beginning of `inventory_state.jsonl` so a
newly connected endpoint sends its current inventory baseline, including inventory
records written before connection. Metadata-only mode applies a local Vector
transform first, removing retained prompts, responses, reasoning, tool
arguments/results, command output, raw fields, diffs, inventory content, and MCP
definitions while preserving stable ids, hashes, counts, actions, paths, token
usage, and other structured metadata. Beacon's [retention and
redaction](/security/retention-redaction) controls still run before either mode.
The ingest service adds tenant identity server-side.
...
Each enrolled machine gets its own key. For user mode, the signed-in CLI account's
`device:enroll` scope authorizes the key without another browser round trip. System
mode retains browser approval. The account token is never written into endpoint
state; the returned device key is stored in a `0600` secrets file that Vector reads
through its secret backend.
...
```bash title="Generate the Beacon pack for a system-mode deployment"
...
3. Within a minute or two, search the dashboard telemetry page for `Beacon endpoint Beacon validation event`.
