...
This is the only Beacon destination that sends telemetry to a Beacon-run endpoint, and it is opt-in. Beacon's default remains local-only; nothing is forwarded until a member of your Beacon organization approves the device in the browser and the forwarder is started with the resulting key. Revoking the device from the dashboard's Beacon Endpoints page stops ingestion within about a minute.
