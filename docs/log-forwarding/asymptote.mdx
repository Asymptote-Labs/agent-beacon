...
Standard mode forwards every line written after the forwarder starts exactly as Beacon wrote it locally. Metadata-only mode applies a local Vector transform first, removing retained prompts, responses, reasoning, tool arguments/results, command output, raw fields, diffs, inventory content, and MCP definitions while preserving stable ids, hashes, counts, actions, paths, token usage, and other structured metadata. Beacon's [retention and redaction](/security/retention-redaction) controls still run before either mode. The ingest service adds tenant identity server-side.
