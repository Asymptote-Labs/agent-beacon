...
This is the only Beacon destination that sends telemetry to a Beacon-run endpoint. It is preselected during interactive onboarding, but nothing is forwarded until the separate connect command mints a device key and starts the forwarder. Choosing Local explicitly opts out. Revoking the device from the dashboard's Beacon Endpoints page stops ingestion within about a minute.
