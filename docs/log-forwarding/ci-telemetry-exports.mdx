...
`--upload` is repeatable, so a job can upload to both destinations when required. Pass `upload: s3,gcs` (or the equivalent `--upload s3 --upload gcs` CLI flags) after configuring credentials for each provider:
...
### Air-gapped and self-hosted binary-path

When runners should not download a Beacon release during the action step, provision `beacon` and `beacon-otelcol` side by side (from an internal mirror or pre-baked image), then pass `binary-path` to the action:

```yaml
- name: Run Claude with Beacon telemetry
  uses: asymptote-labs/agent-beacon@v0.0.95
  with:
    binary-path: /opt/beacon/bin/beacon
    command: claude --print "Review this pull request"
    upload-artifact: "true"
```
...
- [binary-path telemetry workflow](https://github.com/Asymptote-Labs/agent-beacon/blob/main/examples/github-actions/binary-path-telemetry.yml)
