...
Beacon's macOS package and Linux `.deb` and `.rpm` already install Vector at `/opt/beacon/bin/vector`. Otherwise, install Vector using your normal endpoint management tooling. Then copy the generated config into Vector's config directory. On a macOS system-mode Beacon deployment, the generated config tails `/var/log/beacon-agent/runtime.jsonl`:
