...
Interactive installation signs in and preselects Managed. Connect the endpoint
after reviewing the privacy choice:

```bash
beacon endpoint connect
```

The existing CLI account authorizes a separate device ingest key; there is no
second browser approval. Use `--privacy-mode metadata-only` to override Standard.
