...
| [Codex CLI](/runtimes/codex-cli) | Local OTLP logs plus usage reconciliation hooks | Session, prompt, approval, and tool-result activity from Codex semantic logs; session-level token usage from local Codex session JSONL |
