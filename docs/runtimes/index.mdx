...
| [Grok Bot](/runtimes/grok-bot) | Cursor Enterprise OpenTelemetry Export received by a customer-run Beacon collector | Shell commands with the shell-policy decision, MCP tool calls, browser navigation, computer-use sessions, model requests with token usage, and reported cost; no prompt text, tool arguments, or file activity |
