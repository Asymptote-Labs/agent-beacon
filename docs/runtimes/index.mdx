...
- **Browser chat**: An optional MV3 extension for Chromium browsers and beta Firefox reads the chat streams of supported sites and posts them to the same local collector.
...
| [DeepSeek Harness](/runtimes/deepseek-harness) | DeepSeek's own Claude Code hook bridge, mounted by a Beacon row in `$DSH_HOME/cordis.patch.yml` at a hooks file Beacon owns, plus poll of `$DSH_HOME/sessions` through `beacon endpoint dsh sync` | Hooks: session start, prompts, pre-tool and post-tool activity, commands with output and exit code, file reads and edits with diffs, MCP tool calls, subagent start and stop, and the turn-closing stop. Poll: historical prompts, assistant text/reasoning, tool calls/results, failed tools, command/file/MCP activity, and token usage where persisted. No approval decisions, because neither surface exposes them |
...
| [GitHub Copilot CLI](/runtimes/github-copilot-cli) | MDM-managed OTLP HTTP plus poll of `~/.copilot/session-state` through `beacon endpoint copilot sync` | OTLP: prompt, session, tool, and approval-like activity emitted through Copilot CLI spans. Poll: assistant text, complete tool results, commands, files, and token usage from committed session records |
| [goose](/runtimes/goose) | Adapter only today: manually written plugin hooks in `.agents/plugins/`, plus manually configured local OTLP export over HTTP | Hooks: session start and end, prompts, pre-tool and post-tool activity, failed tools, command lines, file creates and edits with diffs, and MCP tool calls. OTLP adds token usage, model and provider, response ids, and agent reasoning. No automatic installer/discovery yet, no tool output of any kind on the hook path, and no approval decisions because goose exposes none of those on a hook |
...
| [Kimi Code](/runtimes/kimi-code) | Native hooks appended to the `[[hooks]]` array of Kimi Code's own `$KIMI_CODE_HOME/config.toml` | Session start and end, prompts, pre-tool and post-tool activity, failed tools, real operator approval decisions, commands with output and exit code, file reads and edits with diffs, MCP tool calls, subagents, and context compaction. No token usage or agent message text, because Kimi Code exposes neither on a hook |
...
|---------|--------------------------|
| Claude Code | The default `beacon endpoint install` detects Claude and configures local OTLP export plus hooks. To backfill committed transcripts, schedule `beacon endpoint claude sync` in the logged-in user's context or leave `--watch` running. |
| Cursor | The default `beacon endpoint install` detects Cursor and installs its hooks. To backfill committed local sessions, schedule `beacon endpoint cursor sync` in the logged-in user's context or leave `--watch` running. |
...
| DeepSeek Harness | Install hooks with `beacon endpoint hooks install --harness dsh`. To backfill committed native sessions, schedule `beacon endpoint dsh sync` in the logged-in user's context or leave `--watch` running. |
| Gemini CLI | The default `beacon endpoint install` detects Gemini and manages `~/.gemini/settings.json` for local OTLP collection. Use an explicit list such as `claude,codex,gemini` when automatic discovery is unavailable. |
...
| GitHub Copilot CLI | Configure launch environment through MDM or customer policy: `COPILOT_OTEL_ENABLED=true` and `OTEL_EXPORTER_OTLP_ENDPOINT=http://127.0.0.1:4318`. Beacon validates the endpoint but does not edit shell profiles or launch environments. To backfill committed sessions, schedule [`beacon endpoint copilot sync`](/cli/endpoint-copilot) in the logged-in user's context or leave `--watch` running. |
...
| Kimi Code | Install user-level hooks with `beacon endpoint hooks install --harness kimi`. Beacon appends its entries to `$KIMI_CODE_HOME/config.toml` without re-serializing the rest of the file. Start a new Kimi Code session or run `/reload` after installation. |
...
The default user-mode install discovers and configures supported user-level hooks, plugins, and extensions. Use `beacon endpoint hooks install` for an explicit target or project-level setup. OpenHands CLI/GUI coverage specifically requires project scope, and goose remains the exception: its adapter can map manually configured hooks, but Beacon does not yet install or discover it. System packages and MDM scripts keep their explicit fleet harness lists because runtime hook configuration belongs to the logged-in user's profile. Poll sync commands are also run or scheduled in the account that owns the runtime's local session store.
