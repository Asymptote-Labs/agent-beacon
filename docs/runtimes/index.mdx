...
| [Prime Agent](/runtimes/prime-agent) | Managed extension hooks | Session lifecycle, prompts, tool lifecycle, commands including each `ipython` kernel cell and operator `!` commands, file edits with the unified patch, agent reasoning, and token usage. No approval decisions, because Prime Agent exposes none; MCP calls are not separately attributed, because it does not expose MCP as agent tools |
...
One row in that table is not a hook or OTLP integration. [fx](/runtimes/vercel-fx) exposes no hook, plugin, or OpenTelemetry surface at all, so Asymptote reads the session records fx commits to disk; every fx event is marked `harness.collection_method=poll`, meaning it was read after the turn was committed rather than observed as the agent worked, and nothing on that path can hold or deny a tool call.
...
| [OpenClaw Gateway](/runtimes/openclaw-gateway) | Managed plugin, plus gateway-configured OTLP/HTTP | Session lifecycle, prompts with the sender and chat channel, tool, command with exit code and output, file with diffs, MCP, compaction, subagent, and token usage telemetry from the plugin; gateway traces and metrics from OpenClaw's own diagnostics export. Approvals are not exposed by OpenClaw and are not synthesized |
