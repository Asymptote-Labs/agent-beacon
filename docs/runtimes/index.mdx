...
| [Pi](/runtimes/pi) | Managed extension | Session lifecycle, prompts, tool calls and results, commands, file reads/edits with diffs, MCP activity, agent reasoning, and per-message token usage and cost |
...
Install Antigravity CLI, Claude Code, Codex CLI inventory hooks, Cursor, Devin CLI, Devin Desktop, Factory, Grok Build, Hermes Agent, OpenCode, Pi, and optional VS Code hooks separately in the logged-in user's context when you need richer hook telemetry or inventory heartbeat triggers.
