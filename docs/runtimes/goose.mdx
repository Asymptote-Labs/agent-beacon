---
title: "goose"
description: "Manual Beacon adapter details for goose endpoint telemetry"
---

## Runtime overview

Beacon currently has a goose payload adapter, but not automatic goose installer
wiring. There are no goose-specific endpoint install, hooks install, or discovery
status paths yet.

Manual goose coverage can combine two collection paths:

- A manually written goose hooks file that invokes `beacon-hooks --platform goose`
  subcommands.
- goose's own OTLP HTTP export pointed at the local Beacon collector.

The two paths are not alternatives. goose's hooks report prompts, tool calls,
command lines and file edits; goose's OpenTelemetry export reports token usage,
the model and provider, response ids and the agent's reasoning. **Neither path
carries the other's data**, so an endpoint with only one configured is partial.

<Warning>
This page documents the current manual adapter contract. Until installer and
discovery support are added, use it as implementation guidance rather than a
turnkey `beacon endpoint install` workflow.
</Warning>
...
Before manually enabling goose telemetry, make sure:

- goose is installed, either the CLI or the desktop app.
- `beacon endpoint install` has run for the endpoint generally so shared endpoint
  config, the collector, and runtime log paths exist. The OTLP path needs the
  collector actually listening; the hook path does not.
...
goose discovers plugins as the **immediate subdirectories** of its plugins directory, names each plugin after its directory, and loads `<plugin-root>/hooks/hooks.json` from every plugin that is enabled. A future Beacon installer should own a **directory** rather than a file, and should not read or write any other plugin beside it.
...
<Warning>
**The two scopes replace each other; they do not add up.** goose deduplicates plugins by **name**, taking project scope first — so a project-scope `beacon-endpoint` plugin replaces the user-scope one for sessions in that directory rather than running alongside it. Beacon installs the same directory name at both scopes deliberately: an operator who installs at both gets **one** registration, not two copies of every event.
...
Today, manually configured goose hooks are not reported as a first-class installed
harness by `beacon endpoint discover` or `beacon endpoint hooks status`.
...
This is a stated limitation with no fix on Beacon's side. It costs nothing today, because goose is the only supported runtime that reads this directory.
</Note>

### Registered events

goose exposes twelve lifecycle events. Beacon registers seven, and the five it leaves out are covered under [Known gaps](#known-gaps).
...
Beacon's hooks carry no `matcher`. The field is a regex tested against goose's `matcher_context`, and that is not a tool name on every event — on `UserPromptSubmit` it is **the prompt text**. A matcher that looked like a harmless `.*` would be a regex run over every prompt, and a narrower one would silently drop prompts that did not match it. Omitting it means always-match, which is what a telemetry hook wants on all seven.

### The hook environment

goose runs a `command` action through `sh -c`, exports `PLUGIN_ROOT`, and sends the event as JSON on stdin. Beacon passes the endpoint log and config locations to `beacon-hooks` as `--log` and `--config` flags rather than through the environment, so a hook finds them wherever it runs.
...
The no-op `{}` that every other runtime reads as "no opinion" lands there, and so does `{"permission":"allow"}` — goose reads `decision`, and accepts only `allow` and `block`. Beacon therefore answers `{"decision":"allow"}` on both blocking events.

**That allow approves nothing.** goose's hook chain is a plugin-policy layer inside `ToolExecutionOperation`, and the pipeline runs `ToolApprovalOperation` *before* it — so the operator's decision has already been made by the time a hook is consulted. It means "this policy hook does not object", and reaches no permission judge. This is the opposite of [Qwen Code](/runtimes/qwen-code), where answering "allow" would disarm the user's own prompts and Beacon therefore answers with an empty object.
</Warning>

### Manual plugin directory

Because goose loads every plugin it discovers, adding a Beacon-shaped plugin
directory is additive by construction: your own plugins keep working, untouched.

If you create the conventional `beacon-endpoint` directory by hand, keep it
dedicated to Beacon hook commands and avoid mixing unrelated hooks into the same
`hooks/hooks.json`. This mirrors the ownership boundary the eventual installer
should enforce.

## Collection path 2: OpenTelemetry export

goose ships a real OpenTelemetry exporter emitting OTel GenAI semantic-convention
spans. Configure it manually by pointing goose's own `config.yaml` at the local
collector HTTP port:
...
### Your config.yaml stays yours

`config.yaml` is a file you write by hand — your provider, your model, your
extensions, your permissions. Beacon does not currently rewrite this file for
goose; add or update the OTLP keys yourself and keep whatever backup practice you
use for manual goose configuration.
...
goose events normalize to the `goose` harness when the manual paths are
configured. Hook events carry `harness.collection_method=hook` because the events,
payload shapes, and hooks schema are goose's. OTLP events carry
`harness.collection_method=otlp`.

<Note>
**A hooks file on disk is a registration, not a guarantee.** goose lets you
disable a plugin from the `plugins` map in `config.yaml` or from
`disabledPlugins` in a `settings.json` at any of three scopes, and a disabled
plugin keeps its hooks file. Run one session and check the log to confirm events
are arriving.
</Note>

## Manual configuration support

Beacon does not yet create, status-check, or uninstall goose hook files. To test
the adapter, write a goose `hooks.json` manually and point hook commands at the
installed `beacon-hooks` binary with `--platform goose`. Use goose's own
configuration to enable OTLP HTTP export to `http://127.0.0.1:4318`.

Commit `.agents/plugins/beacon-endpoint/` only if you intentionally want the hooks
to apply to everyone working in the repository, remembering that project scope
replaces any user-scope plugin with the same name for sessions in that repository.

## Telemetry coverage

| Area | Support |
|------|---------|
| Runtime discovery | Not first-class yet. Manual hooks and OTLP export can write `goose` events, but `beacon endpoint discover` does not install or report goose as a managed target |
...
| Hook install, status, uninstall, and repair | Not yet supported by `beacon endpoint hooks` |
| OTLP export configuration | Manual. Write goose's own `config.yaml` to use the collector HTTP port |
...
| Local JSONL and dashboard | Supported when manual hook or OTLP telemetry reaches Beacon |
| MDM deployment | Supported for the endpoint agent. goose hook and OTLP configuration remain manual until installer support lands |
...
**Beacon records no approval decisions for goose, and does not synthesize any.**
...
The OTLP path carries message content only when `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=true` is set in goose's environment. That is goose's own opt-in and is not written by Beacon — it cannot be: goose reads it from the environment only, never from `config.yaml`.
...
- **No approval decisions.** goose exposes no approval event, and Beacon does not synthesize one from `PreToolUse`. Approval rules and dashboards will show nothing for this runtime.
- **No reported cost.** goose reports token counts and not a price, so `gen_ai.usage.cost_usd` is empty. Beacon does not derive cost from local pricing tables for any runtime.
...
- **No working directory on three events.** goose omits it on `SessionStart` and `SessionEnd` when they come from the agent rather than the CLI session loop, and on a `UserPromptSubmit` raised by steering mid-turn. Those events carry no `session.working_directory` and no repository. Beacon deliberately does **not** fall back to the hook process's own directory: goose runs hooks with `sh -c` inheriting its own cwd, which is the application bundle under the desktop app — a path that looks like a repository and resolves to nothing. Every tool event does carry it.
...
Generate one goose event and check the log:

```bash
beacon endpoint doctor --system
rg '"name":"goose"' ~/.beacon/endpoint/logs/runtime.jsonl
```
