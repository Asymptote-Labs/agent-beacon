...
The runtime log keeps the live file plus five archives, 10 MiB each, and every rotation deletes the oldest archive. A first sync over a long Claude Code history can produce far more than that window. A sweep never rotates out its own output. When writing the next event would delete the file that holds the sweep's first event, the sweep stops. It leaves the cursors of the remaining sessions (including the one it stopped inside) where they were and exits non-zero:
