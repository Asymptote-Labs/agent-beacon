---
title: "Muse Code"
description: "Beacon support details for Meta Muse Code endpoint telemetry"
---

## Runtime overview

Beacon supports Muse Code, Meta's terminal coding agent, through native hook payloads sent to `beacon-hooks`.
...
| File | Owner | Purpose |
|------|-------|---------|
| `~/.config/muse/beacon-endpoint-hooks.json` | Beacon, written outright | The hook registrations themselves |
| `~/.config/muse/settings.json` | You; Beacon edits one key | `managed_hooks_path` points Muse at the file above |

`$XDG_CONFIG_HOME/muse` is used instead when that variable is set, matching how Muse resolves the directory itself.

Install edits `settings.json` by exactly one key. Unrelated settings — including ones Beacon has never heard of — survive the round trip byte for byte. A `settings.json` whose JSON is invalid is reported rather than replaced; its only problem might be a trailing comma.
...
Beacon registers every Muse Code lifecycle event that carries endpoint signal:
...
<Warning>
**Muse hook timeouts are seconds, not milliseconds.** The same `timeout` field is milliseconds on [Qwen Code](/runtimes/qwen-code), in a settings file that looks nearly identical. Beacon writes second values for this runtime. Keep the unit in mind if you hand-edit them.
</Warning>
...
Everything about Muse's managed hooks file rejects quietly. There is no `muse hooks validate` command, and a bad file produces no warning, no exit code and no log line — so a broken registration looks exactly like a working one. Beacon's installer is shaped around that, and each of these is covered by a test:
...
- **Unknown keys inside a group or handler are fatal.** One unrecognized key and that whole event is skipped while the rest of the file keeps working — so a typo removes monitoring from one event and leaves the install looking healthy. `env`, `shell`, and a group's `enabled` are all rejected. Beacon emits only `type`, `command`, `timeout`, and `statusMessage`.
- **Unknown keys at the top level are tolerated**, which is where Beacon's `beacon` marker lives.
...
This costs Beacon nothing, because the endpoint log and config locations are passed to `beacon-hooks` as `--log` and `--config` flags rather than through the environment. It is worth knowing when reading a hook command by hand, and when debugging why a hook cannot find a path you can see from your shell.
...
**Install refuses to take over an existing registration.** Muse reads exactly one managed hooks file, so if `managed_hooks_path` already names somebody else's, Beacon stops and says so rather than replacing it — overwriting would look like a clean install and quietly end their monitoring. The check runs before anything is written, so a refusal leaves nothing behind. The uninstall is the same property in reverse: it clears the key only while it still names Beacon's file, and leaves a hooks file Beacon did not write alone.
...
Muse Code sends the Claude Code payload shape but does not publish its tool names. The names that are confirmed are snake_case (`read_skill`, and the six `subagent_*` control tools), so Beacon matches file-edit tools with a substring rule over `edit`, `write`, `create` and `patch` rather than a guessed literal list — a wrong literal list would capture nothing, silently.
...
Approval telemetry comes from Muse's `PermissionRequest` event and is recorded as `observed`, because an operator was actually asked. Beacon does not synthesize an approval from a pre-tool notification for this runtime — doing so would put an invented decision next to a reported one for the same call.

Beacon's `PreToolUse` hook answers with an empty object, for two independent reasons. Muse reads a tool decision from `hookSpecificOutput`, so answering `allow` would disarm the user's own permission prompt on a runtime the hook was installed only to watch. And Muse rejects a stdout object carrying keys it does not know, so a differently shaped permissive answer would not read as permissive — it would fail the hook run.
...
- Confirm `~/.config/muse/settings.json` has `managed_hooks_path` pointing at Beacon's hooks file, and that the file exists. Either half alone collects nothing.
