...
| [DeepSeek Harness](/runtimes/deepseek-harness) | Looks for the `dsh` executable or `$DSH_HOME` (default `~/.dsh`), then checks both Beacon's owned hook file and its mount row in `cordis.patch.yml` |
| [Kimi Code](/runtimes/kimi-code) | Looks for the `kimi` executable or `$KIMI_CODE_HOME` (default `~/.kimi-code`), then parses `config.toml` and checks for Beacon's `--platform kimi` hook commands |
...
| [Gemini CLI](/runtimes/gemini-cli) | The default endpoint install detects Gemini and writes local OTLP settings to `~/.gemini/settings.json`, sets `target` to `local`, uses OTLP gRPC, enables the collector, and removes `outfile` so telemetry flows to Beacon. |
| [VS Code](/runtimes/vscode) | The default endpoint install detects VS Code, writes Copilot Chat OTel settings to VS Code user settings, points OTLP/HTTP to the local collector, and installs user-level hooks. |
| [GitHub Copilot CLI](/runtimes/github-copilot-cli) | Copilot's OTLP HTTP endpoint remains under MDM or customer-owned launch-environment policy. Beacon validates `COPILOT_OTEL_ENABLED=true` and a localhost OTLP endpoint but does not write Copilot configuration. Committed sessions can be backfilled with `beacon endpoint copilot sync`. |
...
| Hook-based runtimes | The default endpoint install detects and configures supported user-level hooks and managed plugins for [Antigravity CLI](/runtimes/antigravity-cli), [Claude Code](/runtimes/claude-code), [Cline](/runtimes/cline), [Cursor](/runtimes/cursor), [DeepSeek Harness](/runtimes/deepseek-harness), [Devin CLI](/runtimes/devin), [Devin Desktop](/runtimes/devin-desktop), [Factory](/runtimes/factory-droid), [Grok Build](/runtimes/grok-build), [Hermes Agent](/runtimes/hermes-agent), [Kimi Code](/runtimes/kimi-code), [Kiro](/runtimes/kiro), [Muse Code](/runtimes/muse-code), [Oh My Pi](/runtimes/oh-my-pi), [OpenCode](/runtimes/opencode), [Pi](/runtimes/pi), and [Qwen Code](/runtimes/qwen-code). Use [`beacon endpoint hooks`](/cli/hooks) for explicit targets and project scope. OpenHands requires project scope for CLI/GUI coverage, and goose still has no installer. DeepSeek Harness and Kimi Code support user scope only. |
...
| [GitHub Copilot CLI](/runtimes/github-copilot-cli) | MDM-managed OTLP HTTP export plus poll of `~/.copilot/session-state` | Beacon validates Copilot's effective launch environment and maps Copilot spans into prompt, session, tool, and approval-like endpoint events. `beacon endpoint copilot sync` backfills committed session records as poll events with assistant text, complete tool results, commands, file activity, and token usage. |
| [DeepSeek Harness](/runtimes/deepseek-harness) | Native hook bridge plus poll of `$DSH_HOME/sessions` | Beacon installs an owned hooks file and a mount row in the runtime's home-level `cordis.patch.yml`. `beacon endpoint dsh sync` backfills assistant text/reasoning, failures, persisted tool detail, and token usage as poll events. There is no project scope. |
| [Kimi Code](/runtimes/kimi-code) | Native hook payloads through `beacon-hooks` | Beacon appends a verified `[[hooks]]` block to the runtime's existing `config.toml` without reserializing the file. Hooks cover lifecycle, prompts, tools, commands, files, MCP, subagents, compaction, and observed permission decisions. Kimi exposes no token usage on hooks and has no project scope. |
...
| [Browser Extension](/runtimes/browser-extension) | Managed browser extension over local OTLP | An MV3 extension for Chromium browsers and beta Firefox parses the [claude.ai](/runtimes/claude-web) and [chatgpt.com](/runtimes/chatgpt-web) chat streams and posts OTLP GenAI logs to the local collector. Installed unpacked rather than from a browser store, and configured in the extension's own options page rather than by `beacon`. |

## Hook telemetry

Antigravity CLI, Claude Code, Cline, Cursor, DeepSeek Harness, Devin CLI, Devin
Desktop, Factory, goose, Grok Build, Hermes Agent, Kimi Code, Muse Code, Oh My
Pi, OpenCode, OpenHands, Pi, Qwen Code, and optional VS Code hooks cover activity
that is better represented as runtime events than generic OTLP spans.
...
| [DeepSeek Harness](/runtimes/deepseek-harness) | Session lifecycle, prompts, tools, commands, files, MCP and subagent activity through the Claude-shaped bridge. The live bridge does not expose assistant text/reasoning, failures, or token usage; session sync fills those gaps. |
| [Kimi Code](/runtimes/kimi-code) | Session lifecycle, prompts, tool invocation and failures, commands with output and exit code, file reads and edits with diffs, MCP calls, subagents, context compaction, and observed permission decisions. No hook payload exposes token usage. |
