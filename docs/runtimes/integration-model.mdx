...
| [Prime Agent](/runtimes/prime-agent) | Managed local extension through `beacon-hooks` | The other pi-mono fork, sharing Pi's event payload shapes and so Pi's mapper, but never Pi's identity: separate harness name, separate extension marker, separate install directory. Its single-tool design is what makes it different to read -- the agent works through one persistent Python kernel rather than separate file and shell tools, so a cell is recorded as a command and the diffs the kernel streams become one file event per changed file. Like Pi, it exposes no operator approval decision, and Beacon does not synthesize one. |
