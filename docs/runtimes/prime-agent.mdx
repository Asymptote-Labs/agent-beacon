---
title: "Prime Agent"
description: "Beacon support details for Prime Agent (Prime Intellect) endpoint telemetry"
---
...
- **No approval telemetry.** Prime Agent's `tool_call` event lets an extension block a call, but that is an extension deciding rather than an operator being asked. It exposes no operator approval decision through the extension API, so Beacon records these as tool activity and writes no approval events. [Pi](/runtimes/pi) and [Cline](/runtimes/cline) work the same way for the same reason; [Oh My Pi](/runtimes/oh-my-pi) is the fork that does report real ones. Detections in `rules/approval-abuse/` do not fire on Prime Agent activity.
