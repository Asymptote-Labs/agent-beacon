...
The Firefox build is in beta. It is generated from the same sources as the Chrome build and passes Mozilla's linter in CI, but it has no automated end-to-end test, so capture on Firefox is checked by hand.
