...
Beacon collects browser chat telemetry through an optional Chrome MV3 extension. It reads the chat streams of the sites you enable and relays them into the same local pipeline as agent activity, so browser conversations land in the runtime log next to CLI agent events.
...
- You have Chrome, Edge, or another Chromium browser that can load an unpacked MV3 extension. See [Chromium browsers](/runtimes/browser-extension-chromium) for which ones are tested. Firefox and Safari are not supported.
...
| MDM deployment | Not covered by Beacon's MDM assets. Browser force-install policy needs a store listing or signed package, which do not exist yet. See [Force-install through browser policy](/runtimes/browser-extension-chromium#force-install-through-browser-policy) |
