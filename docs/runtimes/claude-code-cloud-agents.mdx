...
description: "Capture Claude Code cloud agent telemetry and send each session log to Beacon Managed or to object storage you own"
---

## Overview

Use this integration to capture Claude Code cloud-agent activity and send each
session log to one of three places: [Beacon Managed](/deployment/managed),
...
ships that file: to Beacon Managed it posts the new lines to the same ingest
...
flowchart LR
  ...
  RuntimeLog --> Managed["Beacon Managed ingest → dashboard"]
  RuntimeLog --> Storage["Your GCS or S3 bucket"]
```

The setup has three parts:

1. Get a device key from Beacon, or create a GCS or S3 upload path with Beacon.
...
With Beacon Managed, the sessions land in the same per-organization tables as
...
- A Claude cloud environment with outbound access to:
  - For Beacon Managed: the ingest host from your device key hand-off
...
Choose Beacon Managed, GCS, or S3 for the cloud-agent session logs.
...
a device key that a Beacon operator mints for the environment itself: one
...
Beacon contact for a key, naming the environment (for example
...
For Beacon Managed, open `/dashboard/endpoints`: the cloud environment's
...
The Beacon Managed flow puts a per-device key in the Claude Code environment.
