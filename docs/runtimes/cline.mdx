---
title: "Cline"
description: "Beacon support details for Cline endpoint telemetry"
---

## Runtime overview

Beacon supports [Cline](https://cline.bot/) through a managed local plugin that invokes `beacon-hooks`.

Cline is not a single runtime. The same agent core runs as a VS Code extension, a JetBrains plugin, and a CLI, and Beacon's plugin covers all of them: it is loaded by whichever host is running, so one install collects telemetry from every surface on that machine.
...
- Beacon's Cline plugin is installed with `beacon endpoint hooks install --harness cline`.
...
Beacon installs an owned Cline plugin at `~/.cline/plugins/beacon.ts` for user-level hooks or `./.cline/plugins/beacon.ts` for project-level hooks. Cline auto-discovers plugin files in those directories, so no plugin registry command is required. That is also why the install works for the VS Code and JetBrains hosts, neither of which ships a `cline` executable.

The plugin forwards `beforeRun`, `beforeTool`, `afterTool`, and `afterRun` hook payloads to Beacon's hook adapter. It spawns the adapter directly with an argument vector rather than through a shell, so the same plugin works unchanged on macOS, Linux, and Windows.
...
Beacon can also recover Cline activity from Cline's committed local session stores:

```bash title="Sync historical Cline sessions"
beacon endpoint cline sync
```

The sync path reads `~/.cline/data/tasks`, `~/.cline/tasks`, Cline's VS Code global-storage task directory, `~/.cline/data/sessions`, and `~/.cline/kanban/workspaces`. It is local and offline, emits events with `harness.collection_method=poll`, and is useful for historical sessions or sessions that ran before the Beacon plugin was installed. Use `--print` to inspect mapped events without writing the runtime log or advancing the cursor.

## Discovery and status

Beacon detects Cline through the `~/.cline` directory, treating a `cline` executable on `PATH` as an additional signal rather than the deciding one. A binary check alone would report "not detected" for the IDE hosts, which are the majority of installs.
...
Use `beacon endpoint cline status` to inspect Cline session stores and how much of each discovered trace Beacon has already synced.
...
| Model and usage | Model and provider (recorded as `model` and `gen_ai.provider.name`), token counts, cache reads, and runtime-reported cost, recorded once per task |
| Approval telemetry | Not supported. See [Known gaps](#known-gaps) |
| Local JSONL and dashboard | Supported |
| Historical session sync | Supported through `beacon endpoint cline sync`; marked as poll collection |
...
The session sync path reads what Cline has already committed to disk. It can recover prompts, assistant messages, agent reasoning, tool calls/results, command/file/MCP-shaped tool activity, model ids, and token counts where the stored history carries them. It cannot hold or deny tool calls, and it does not imply that Beacon observed the event at runtime.
