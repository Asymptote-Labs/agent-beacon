...
Beacon supports DeepSeek Harness — DeepSeek's open agent harness, run as `dsh` — through the Claude Code hook bridge DeepSeek ships in the CLI, sent to `beacon-hooks`.
...
Uninstall removes Beacon's hooks file and its one patch element, and leaves your rows and comments alone.
...
- **No approval decisions.** The bridge exposes no `PermissionRequest` event, and Beacon does not synthesize one from `PreToolUse`. Approval rules and dashboards will show nothing for this runtime.
