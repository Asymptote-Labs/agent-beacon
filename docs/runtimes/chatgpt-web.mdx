---
title: "ChatGPT"
description: "Beacon support details for ChatGPT browser chat telemetry"
---

## Runtime overview

Beacon collects ChatGPT chat telemetry through the optional [browser extension](/runtimes/browser-extension). Every turn on chatgpt.com becomes prompt and response events in the local runtime log, attributed to the `chatgpt_web` harness so it stays separate from Codex CLI and other agents.
...
| Stream signal | What Beacon takes from it |
|---------------|------------------------------|
...
- **No token usage.** Nothing in the stream carries token counts, and Beacon does not estimate them.
