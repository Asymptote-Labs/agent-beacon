...
Enabling scheduled endpoint updates creates
`/Library/LaunchDaemons/com.beacon.endpoint.updater.plist`. Scheduled or manual
update activity writes `/var/log/beacon-agent/system.jsonl`. Self-updates are
off by default.
...
<Steps>
  ...
  <Step title="Scope repair workflows">
    Use a root MDM workflow that invokes `beacon endpoint repair --system` for endpoints where inventory reports a stale or unhealthy install. Repair stops the existing collector before reinstalling it and restores the previous endpoint configuration if reinstall cannot complete.
  </Step>
...
Connecting packaged endpoints to Beacon Managed is a separate, interactive step today: an admin runs `sudo /opt/beacon/bin/beacon endpoint connect --system --dashboard-url https://beacon.sh` on the machine and the approval page opens in the console user's browser. The package never enrolls a device on its own, and there is no headless enrollment for MDM fleets yet; it is a planned follow-up. See [`beacon endpoint connect`](/cli/endpoint-connect).
...
| `BEACON_HOOK_HARNESSES` | Optional user-context hooks such as `antigravity,claude,cline,cursor,devin,factory,grok,hermes,opencode,qwen` |
...
<Columns cols={2}>
  ...
  <Card title="Rippling" icon="laptop" href="/mdm/rippling">
    Deploy Beacon with custom installers, Supergroups, and recurring scripts in Rippling Device Management.
  </Card>
  ...
  <Card title="Beacon with Fleet and S3" icon="bucket" href="/guides/fleet-s3-mdm">
    Install Beacon from Fleet and forward runtime plus inventory JSONL to S3.
  </Card>
