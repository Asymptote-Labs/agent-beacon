...
<AccordionGroup>
  ...
  <Accordion title="Runtime events are stale or missing">
    ...
    If validation succeeds but events remain stale, confirm the configured harnesses match the agent harnesses installed or configured for the device and that the local collector ports are not in use by another process. If GitHub Copilot CLI events are missing, confirm Copilot's launch environment includes `COPILOT_OTEL_ENABLED=true` and a localhost OTLP HTTP endpoint. If Cline, Cursor, Devin CLI, Devin Desktop, Factory, Grok Build, Hermes Agent, or OpenCode hook events are missing, confirm the separate user-context hook policy has run for the logged-in user. For OpenClaw Gateway, confirm the Beacon plugin is present in `~/.openclaw/extensions/beacon-endpoint/` and that the gateway was reloaded, and — for the separate gateway export — that the `diagnostics-otel` plugin is enabled and pointed at Beacon's OTLP HTTP receiver.
  </Accordion>
  <Accordion title="Agent inventory heartbeats are stale">
    System-mode inventory heartbeats are written by a scheduled root job,
    `com.beacon.endpoint.inventory`, every 6 hours and once at load, to
    `/var/log/beacon-agent/inventory_state.jsonl`. The package postinstall
    installs and loads it on every upgrade; user hooks no longer trigger
    inventory, so no user needs write access to the system log files and the
    older chown/chmod remediation is not needed.

    Confirm the job is loaded and when it last wrote:

    ```bash
    sudo launchctl print system/com.beacon.endpoint.inventory | grep -E 'state|last exit'
    sudo /opt/beacon/bin/beacon endpoint status --system | grep 'Inventory heartbeat'
    sudo tail -n 2 /var/log/beacon-agent/inventory_state.jsonl
    ```

    If the job is missing, reconcile it (this also writes a heartbeat at once):

    ```bash
    sudo /opt/beacon/bin/beacon endpoint inventory install-daemon --system
    ```

    The job inventories the active console user's home. A machine with nobody
    logged in still heartbeats, with `user_scope.mode` set to `no_console_user`,
    and writes a snapshot the next time a user is at the console.
  </Accordion>
