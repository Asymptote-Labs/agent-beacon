...
For S3 forwarding plus self-updates, follow [Beacon with Fleet and S3](/guides/fleet-s3-mdm).

## Deploy with Fleet

Upload the signed and notarized `.pkg` as Fleet software and scope it to a pilot team or label. The package postinstall performs the default system install, so no post-install script is required for the common deployment path. See [Deploy software](https://fleetdm.com/guides/deploy-software-packages) for Fleet's package, script, and targeting model.
...
<Steps>
  ...
  <Step title="Optionally configure S3 forwarding">
    After the package is installed, run a Fleet script that starts Vector
    forwarding. Do not put that script in the package post-install. See
    [Beacon with Fleet and S3](/guides/fleet-s3-mdm).
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="S3 Vector forwarder is not running">
