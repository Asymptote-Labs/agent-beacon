...
### v1.3.22 · September 23, 2026

- **Vector ships with Beacon on Linux** · The `.deb` and `.rpm` install
  [Vector](https://vector.dev) 0.56.0 at `/opt/beacon/bin/vector`, the Linux
  archives carry it as `beacon-vector` beside `beacon`, and Homebrew on Linux
  installs that same file. It is the static musl build, so it runs on minimal and
  custom-built hosts, and it runs only when a destination needs it: Beacon
  Managed, S3, GCS, CloudWatch, Sumo Logic, Rapid7, Falcon, or Sentinel. Beacon
  finds it without PATH or `BEACON_VECTOR_BIN`.
- **Vector is pinned to 0.56.0 everywhere** · Vector 0.57 stops expanding
  `${VAR:-default}` in config files and 0.58 stops expanding `${VAR}`, which broke
  most generated forwarding configs. The Homebrew `beacon-vector` keg on Apple
  Silicon moves back from 0.58.0 to 0.56.0 on the next `brew upgrade`, matching the
  macOS package and the Linux packages. CI now loads every generated config with
  the bundled Vector, and a macOS release fails if its package has no Vector.
- **Microsoft Sentinel without Azure Arc** · The Sentinel pack adds `vector.toml`
  and `dcr-logs-ingestion-template.json`. The bundled Vector sends
  `runtime.jsonl` to the Logs Ingestion API as an Entra app registration, through
  the same transform into the same `BeaconRuntime_CL` table as the Azure Monitor
  Agent path. The docs now say plainly that Azure Monitor Agent on a machine
  outside Azure needs Azure Arc first, and give a systemd unit for the Vector
  path.
- **Sign-in says at once when it cannot finish** · On Linux with no display, or
  over SSH, the install wizard goes straight to recovery and explains why
  instead of waiting five minutes for a browser nobody can see. Finishing without
  an account is the default; the sign-in URL comes with the `ssh -L` command for
  its port. `beacon login` prints the same, and `dashboard --open` no longer fails
  over SSH.
- **Homebrew upgrades no longer strand services** · Vector and the collector
  found beside a Homebrew-installed `beacon` now resolve to the `<prefix>/bin`
  link rather than the versioned keg, so the forwarder and collector units keep
  working after `brew upgrade`.
- **Hook-only installs leave Claude Code and Codex alone** ·
  `beacon endpoint install --harness omp` (or any selection with no OpenTelemetry
  runtime) no longer writes OTLP settings into `~/.claude/settings.json` and
  `~/.codex/config.toml`. Selections that name those runtimes are unchanged.
- **RPM coverage** · The package smoke test now installs, upgrades, and removes
  the `.rpm` on Rocky 9 in CI, alongside the `.deb` on Ubuntu, and checks that
  `apt purge` removes config and logs.

### v1.3.21 · September 23, 2026

- **Browser events identify the browser that produced them** · The browser
  extension now records `user_agent.name` and `user_agent.version` on every
  event, using UA Client Hints with a user-agent fallback. Chromium, Chrome,
  Edge, Brave, Opera, Opera GX, Vivaldi, Arc, and Firefox are distinguished
  where the browser exposes enough information; the typed fields survive
  metadata-only retention. The replay e2e now runs in Microsoft Edge as well as
  Chromium.
- **A beta Firefox browser-extension build** · `npm run build:firefox` produces
  a Gecko-targeted build with a pinned add-on ID, Firefox-compatible background
  and options declarations, Mozilla linting in CI, and a visible site-access
  warning and grant flow when Firefox withholds MV3 host permissions. Permanent
  installation still requires an AMO-signed XPI; the release workflow publishes
  one only when its AMO credentials are configured.
- **Safari packaging groundwork, not Safari support** · The extension has an
  experimental Safari build target plus scripts for generating an Xcode app
  wrapper, validating an MDM declaration, and running packaging checks. It has
  not been tested in Safari, is not wired into a release, and is not shipped.
- **User and system collectors can run together** · A collector's default
  health-check port now moves with an overridden OTLP HTTP port, so a user-mode
  collector on alternate OTLP ports does not collide with a system collector
  still using `13133`. The resolved port is stored in endpoint configuration
  and used by status, diagnostics, repair, and CI checks.
- **Interrupted managed reconnects are no longer reported as healthy** · As soon
  as enrollment returns a rotated device key, connect records a non-secret
  pending marker. If storing the key, writing configuration, or starting the
  forwarder then fails, status reports `connect_incomplete: true` and the retry
  command instead of presenting the previous enrollment as connected. A failed
  first connect also removes any partial forwarder.
- **Large Claude Code backfills preserve the sweep they just wrote** ·
  `beacon endpoint claude sync` stops before rotation would delete output from
  the current sweep, leaves remaining session cursors pending, and reports
  retained and rotated event counts. `--rotate-bytes` can temporarily enlarge
  the sync's rotation window for a one-pass backfill.
- **Existing Claude Code hooks survive Beacon edits verbatim** · Install, repair,
  and uninstall now preserve non-Beacon `command`, `http`, `prompt`, and `agent`
  hooks, including fields Beacon does not interpret such as URLs and headers.
- **DeepSeek sandbox capture gaps are visible** · A hook that cannot write the
  runtime log now warns on stderr without blocking the agent. Endpoint doctor
  compares DeepSeek's native sessions with retained live-hook evidence and
  reports `dsh_hook_capture` when sandboxing prevented live capture, while
  directing operators to session sync for backfill.
- **The supervised collector no longer signals an unrelated process** · In
  containers and other hosts without a service manager, a pidfile that outlived
  its collector could name a process the kernel had since given that pid, and
  unload would send it SIGTERM. The pid now counts as the collector only when its
  command line matches the program it was started as. The RPM uninstall docs now
  say to run `beacon endpoint uninstall --system` before `dnf remove`, since RPM
  has no purge.
- **Trace and memory content fixes** · Trace views now use retained assistant
  response or reasoning text as `content.text`, keeping the event label as the
  summary. Jev/Noul candidates no longer store the question type as if it were
  an evaluator-provided reason.
- **Safer supervised service cleanup** · The no-init-system backend verifies
  that a pidfile still names the collector it started before signaling it, so a
  recycled process ID cannot cause Beacon to stop an unrelated process. Linux
  removal guidance now also makes clear that RPM has no purge operation and
  that a full removal runs `beacon endpoint uninstall --system` before
  `dnf remove`.

### v1.3.20 · September 22, 2026

- **Endpoint install now discovers local runtimes by default** · With no
  `--harness` flag, `beacon endpoint install` and `repair` detect the invoking
  user's runtimes and configure every integration Beacon can safely install,
  including supported OTLP settings, hooks, plugins, and extensions. An explicit
  comma-separated list keeps exact-selection behavior, while `--harness all`
  forces every installable integration.
- **Manual and poll-only integrations are reported, not misconfigured** ·
  Discovery explains the next step for GitHub Copilot CLI, Claude Cowork, fx,
  and project-scoped OpenHands instead of treating those surfaces as install
  failures. If no automatically installable runtime is found, Beacon still
  installs the collector without changing runtime configuration.

### v1.3.19 · September 21, 2026

- **Onboarding happens in one screen that never disappears** · Signing in used to
  quit the full-screen wizard, print to plain output, block for up to five minutes,
  then start a second wizard that wiped what it had printed. It now runs inside the
  same view, which shows the sign-in URL, a spinner and the time remaining. The URL
  is printed before any browser is opened, so it works when none can be, and
  <kbd>esc</kbd> during the wait returns to the sign-in screen instead of ending the
  install. `beacon endpoint install --no-browser` starts in URL mode.
- **Choosing Beacon Managed connects the endpoint** · Confirming Managed installs
  Beacon and then enrolls the device and starts the forwarder, rather than printing
  `beacon endpoint connect` for you to run. The confirm screen states that forwarding
  starts and names what the selected privacy mode sends, so that screen is where
  forwarding is authorized. An endpoint already enrolled is never re-enrolled, since
  enrollment rotates its device key. A failed enrollment leaves a working local
  install, prints the retry command, and re-offers the question next time.
- **A sign-in that cannot finish no longer blocks the install** · Offline, without a
  browser, or after the wait expires, the wizard offers to retry, to show the URL to
  open yourself, or to finish setup without an account. Signing in stays the expected
  path: that screen offers no alternative until an attempt has actually failed.
- **Clearer, shorter onboarding copy** · The wizard explained itself twice and
  repeated the same reassurance on three screens. The destination choice now
  describes what each option gives you: Managed as free, unlimited cloud-based
  retention of every agent session with AI-powered search and analytics on Beacon's
  hosted dashboard, Local as a dashboard for this machine where nothing leaves it,
  each saying when to pick it. The managed disclosure is plain language.
- **A failed install no longer marks the machine onboarded** · The onboarding record
  was written before `lifecycle.Install` ran, so an install that then failed left a
  profile saying setup was complete: the retry skipped the wizard silently and never
  offered the destination again. It is now written only after the install succeeds.
- **A redirected install no longer launches the wizard** · Terminal detection used a
  character-device test, which `/dev/null` satisfies, so an install with stdin
  redirected from `/dev/null` started the full-screen wizard against an input already
  at end of file and failed with "onboarding cancelled". It now uses a real terminal
  check, and `TERM=dumb` takes the same silent skip.
- **The browser page after sign-in is a real page** · Signing in used to land on
  unstyled markup. It now carries Beacon's mark and styling and says the one thing
  that matters: return to your terminal, which is still waiting. Device approval gets
  the same page. Everything is inline, since it is served from a loopback port with
  no network guarantee behind it.
- **Setup ends on what to do next** · A managed install closed on a wall of paths and
  a sentence pairing the dashboard with a revocation link. It now says to run an
  agent as usual and that its sessions appear on the dashboard within a minute.
- **Printed dashboard links point somewhere that exists** · The dashboard is a single
  page at `/dashboard` whose views are tabs, so deep links such as
  `/dashboard/telemetry` and `/dashboard/endpoints` have no route behind them.
  Connect, disconnect, `asymptote validate` and the cloud-agents guide named those
  paths; all now point at the dashboard itself.
- **`q` no longer quits the onboarding wizard** · It was bound to quit on every
  screen, so one stray keystroke on a choice list ended the install. Cancel with
  <kbd>esc</kbd>.
- **Dashboard and MCP HTTP servers reject non-loopback Host headers** · Both bind to
  loopback, which keeps other machines out but not a web page that DNS-rebinds its
  own hostname to `127.0.0.1`; the browser then treats the local server as
  same-origin and the page can read captured prompts and tool activity. Both now
  answer 403 unless the Host header names localhost or a loopback IP.
- **Long traces keep their tail in the learning projection** · Only the first 80
  events were kept, so a long session was evaluated without its outcome, which is
  usually where the fix, the passing test or the final decision lands. The opening
  context and the tail are both kept, with an explicit marker for the omitted middle.
- **DeepSeek Harness sessions with a format version are discovered** · DSH names a
  record `session.v3.jsonl.zstd`, and discovery compared the filename for equality
  against the unversioned names, so a store of versioned records yielded nothing and
  reported that as success. Names are now matched by shape, with the version optional.
- **Claude Code and Codex hooks install by default** · `beacon endpoint install` and
  `beacon endpoint repair` now write each runtime's user-level hooks together with its
  OpenTelemetry settings whenever the `claude` or `codex` harness is selected, which the
  default harness list does. Their OTLP export has no session start or end, file reads or
  diffs, or subagent lifecycle, so a default install previously recorded thinner sessions
  for them than for every other supported runtime. Package installs already refreshed
  these hooks for the console user; this brings Homebrew and tarball installs in line.
  `beacon endpoint hooks uninstall --harness claude` removes them, and `--harness codex`
  the Codex ones.
- **Hook install failures no longer fail the install** · When `endpoint install` or
  `endpoint repair` cannot write a runtime's hooks, it prints a warning and the
  `beacon endpoint hooks install --harness …` command to retry, and still attempts every
  other hook target. The collector, config, service, and OTLP settings are already in place
  at that point, so the endpoint is installed rather than reported broken.

### v1.3.18 · September 21, 2026

- **Kimi Code telemetry** · Kimi Code (Moonshot AI) now has managed hook install,
  status, uninstall, repair, discovery, inventory and dashboard coverage under the
  `kimi_code` canonical name. Beacon registers thirteen of the runtime's twenty
  lifecycle events in the `[[hooks]]` array of `$KIMI_CODE_HOME/config.toml`,
  appending its block and leaving the rest of the file -- including the provider
  API keys it holds -- byte for byte. Coverage includes sessions, prompts, tool
  activity, failed tools, commands with output and exit code, file reads and edits
  with diffs, MCP calls, subagents, context compaction, and real operator approval
  decisions from the `PermissionRequest`/`PermissionResult` pair. Token usage is
  not collected, because no Kimi Code hook payload carries one.
- **Interactive local trace browser** · `beacon traces` opens a full-screen,
  read-only browser over the local trace index, with trace search, event-type
  filters, paging, and event detail. Redirected input or output falls back to a
  bounded tab-separated list, while `beacon endpoint traces` remains the stable
  JSON and scripting interface.
- **Safer relayed memory project resolution** · Dashboard and MCP memory lookups
  resolve relayed paths only against projects already stored in local memory,
  choose the most-specific matching project, avoid filesystem and Git reads, and
  fail closed when no known project matches.
- **Account sign-in and safer first-run onboarding** · `beacon login`, `whoami`,
  and `logout` add browser PKCE account authentication without enabling forwarding.
  Interactive user installs now use a full-screen wizard that requires sign-in,
  preselects Beacon Managed, offers Standard or Metadata-only privacy, and records
  intent without moving telemetry. System, package, MDM, CI, dry-run, and
  redirected-input installs remain noninteractive and account-free.
- **Beacon Managed privacy modes** · Managed forwarding can now use `standard` or
  `metadata-only`. Metadata-only removes retained prompts, responses, reasoning,
  tool arguments and results, command output, raw fields, diffs, inventory
  content, and MCP definitions locally before buffering or upload. Connect now
  validates the rendered Vector configuration before enrollment can rotate a
  device key, and privacy-mode changes clear data buffered under the old mode.
- **DeepSeek Harness session backfill** · `beacon endpoint dsh sync` and `status`
  read committed native sessions from `$DSH_HOME/sessions`, adding assistant text
  and reasoning, failed tools, persisted command/file/MCP detail, and token usage
  that DeepSeek's live hook bridge does not expose. Backfilled events are marked
  `harness.collection_method=poll` and do not synthesize approvals.
- **GitHub Copilot CLI session backfill** · `beacon endpoint copilot sync` and
  `status` read committed records under `~/.copilot/session-state`, complementing
  live OTLP with assistant text, complete tool results, commands, file activity,
  and persisted token usage. Poll events remain offline, deterministic, and
  cannot approve, deny, or delay tools.


### v1.3.17 · September 21, 2026

- **Reviewed cross-harness memory** · Beacon now has an explicit local learning
  workflow under `beacon memory`. Evaluations select local traces, optionally call
  TypeSafe Jev's System One API with a bounded redacted projection, and persist
  results in a local `memory.db` beside the endpoint log base directory. Reviewers
  can list, inspect, approve, reject, or supersede memory candidates before any
  project memory is created.
- **Approved memory reuse through MCP and Agent Skills** · `beacon mcp serve` now
  exposes approved-memory tools (`search_memory`, `get_memory`, and
  `get_memory_context`) alongside the existing local activity tools. Approved
  memory can also be previewed or installed as deterministic Agent Skills with
  `beacon memory skills preview|install`, writing to `.agents/skills/<slug>/SKILL.md`
  only through an explicit CLI action.
- **Memory dashboard and docs** · The local dashboard adds a read-only Memory page
  for evaluations, candidates, and approved memories. The cross-harness memory
  concept and CLI reference document the Jev boundary, project scoping, MCP reuse,
  and Agent Skill publishing.

### v1.3.16 · September 20, 2026

- **Session-centric local dashboard** · The dashboard root now groups local runtime
  activity by agent session, showing prompt, risk, harness, model, repository,
  duration, activity counts, and last-artifact summaries. Session detail pages add
  timelines, severity and type filters, metadata summaries, raw JSON drawers, and
  value inspectors while staying loopback-only and read-only.
- **Dashboard navigation refresh** · Local investigation views now separate Agent
  Activity Sessions, Agent Activity analytics, Findings, Detections, Token Usage,
  Agent Inventory, and Memory so rollout validation and local triage no longer
  depend on a flat event table.
