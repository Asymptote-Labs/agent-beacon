...
- **Codex session token attribution** · Codex hooks now reconcile local Codex session JSONL into token-only `gen_ai.usage` events in the runtime log, giving `beacon token-usage`, the dashboard, and S3 forwarding session-level Codex token attribution while keeping Codex cost out of scope.
