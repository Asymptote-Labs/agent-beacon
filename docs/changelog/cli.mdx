...
- **Scheduled inventory on shared Macs** · A system-mode heartbeat inventories whoever owns the
  console. macOS service accounts (`_mbsetupuser` at the login window, home `/var/empty`) passed
  that check and produced an empty snapshot, and on a Mac where two people alternate at the
  console every switch wrote a "changed" snapshot although nobody's inventory changed. Service
  accounts are now treated as no console user (heartbeat only, no snapshot), and the state file
  remembers the last digest per home, so returning to an unchanged user writes a heartbeat only.
