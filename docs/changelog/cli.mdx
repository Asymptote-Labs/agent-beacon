...
- **`endpoint uninstall` restores hook files the install rewrote** · A user-mode
  `beacon endpoint install` on a machine that also has a system install rewrote the console
  user's Codex hooks to point at the user log, and `uninstall --user` left them that way, so
  Codex telemetry silently went to a log nothing read. Settings-style hook files (Claude Code,
  Codex, Factory, Qwen) now keep a `<file>.beacon.<timestamp>.bak` beside them when an install
  changes them, the install manifest records it, and `endpoint uninstall` puts the previous
  hooks back. An install that changes nothing writes nothing.
- **Homebrew upgrades and the scheduled inventory job** · `brew upgrade` does not re-run
  `endpoint install`, and Homebrew's sandbox does not let a formula install a LaunchAgent, so
  the formula now says it: run `beacon endpoint repair --user` once after upgrading to refresh
  the job.
...
- **Onboarding asks where telemetry should go** · The first-run prompt ends with
  ...
  skips the question and connects, `BEACON_MANAGED_INGEST=0` hides the Asymptote
  answer, the answer stays local and is shown by `beacon endpoint onboarding`; the
  Asymptote answer is recorded only once the machine is connected. The Homebrew
...
- **`beacon endpoint connect` and `disconnect`** · Connect an endpoint to
  Asymptote managed ingest from the CLI: it finds Vector (0.56+), opens the
  ...
  mode. `beacon endpoint status` gains an "Asymptote managed ingest" line and a
...
- **Asymptote Managed forwarding pack** · `beacon endpoint asymptote` generates a
  Vector `http` forwarder that posts the runtime and inventory JSONL to
  Asymptote's ingest service with a per-device key read from a `0600` secrets
  file, plus a credential smoke test and sample events. It is the first Beacon
  destination that sends events to an Asymptote-run service; forwarding is
...
- **Removed `beacon login` and `beacon ingest`** · Neither command was ever
  ...
  Forwarding to Asymptote returns as an explicit, revocable Vector destination
