...
description: "Choose a Beacon Observe integration path for AI SDKs and agent orchestration"
---

## Integration overview

Beacon Observe traces supported LLM providers, agent SDKs, and orchestration code through OpenTelemetry. Initialize `Observe`, configure the integration path for the SDK you use, and your model calls or agent turns can flow to Beacon Managed hosted Observe or a customer-managed OTLP collector.
...
<Columns cols={2}>
  <Card title="Anthropic" icon="robot" href="/sdk/integrations-anthropic">
    Instrument `@anthropic-ai/sdk` model calls with Beacon Observe.
  </Card>
  ...
  <Card title="OpenAI" icon="brain" href="/sdk/integrations-openai">
    Instrument `openai` package model calls with Beacon Observe.
  </Card>
...
Hosted Observe export requires a Beacon Managed API key. To get `ASYMPTOTE_API_KEY`, [reach out for a demo](https://asymptotelabs.ai/contact).
...
See the [SDK quickstart](/sdk/quickstart) for Beacon Managed hosted Observe, customer-managed OTLP, custom exporter, and shutdown options.
