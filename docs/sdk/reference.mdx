---
title: "SDK Reference"
description: "Beacon Observe TypeScript API, options, and Beacon compatibility attributes"
---
...
Function exports are also available for the same operations, including `initialize`, `getTracer`, `observe`, `patch`, `initializeBeaconInstrumentations`, `wrapClaudeAgentQuery`, `flush`, `shutdown`, `isInitialized`, and `resolveExporterConfig`.

## Initialize options

| Option | Purpose |
| --- | --- |
| `apiKey` | Beacon Managed hosted Observe API key. Defaults to `ASYMPTOTE_API_KEY` |
| `baseUrl` | Beacon Managed hosted Observe base URL. Defaults to `ASYMPTOTE_BASE_URL` or `https://api.asymptotelabs.ai` |
...
| --- | --- |
| `hosted` | Beacon Managed `apiKey` or `ASYMPTOTE_API_KEY` is present and no explicit OTLP endpoint is configured |
...
<Columns cols={2}>
  ...
  <Card title="Vercel AI SDK" icon="bolt" href="/sdk/integrations-vercel-ai-sdk">
    Pass the Beacon tracer into AI SDK telemetry.
  </Card>
</Columns>
