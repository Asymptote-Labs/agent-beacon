---
title: "OpenAI"
description: "Instrument OpenAI SDK calls with Beacon Observe"
---

## Integration overview

Use this integration when your application calls the `openai` JavaScript package directly. Beacon Observe uses OpenLLMetry's OpenAI instrumentation so OpenAI spans flow through the same exporter and Beacon-compatible attribute path as the rest of your agent telemetry.

## Overview

Initialize Beacon Observe before creating the OpenAI client when possible. The instrumentation captures supported OpenAI SDK spans without changing the way you call `openai`.
...
- `openai` installed.
- `ASYMPTOTE_API_KEY` set for Beacon Managed hosted Observe, or `OTEL_EXPORTER_OTLP_ENDPOINT` set for customer-managed OTLP export. To get a Beacon Managed API key, [reach out for a demo](https://asymptotelabs.ai/contact).
...
```bash title="Set Beacon Managed and OpenAI environment variables"
...
All supported OpenAI SDK calls are now traced through the configured Beacon exporter.
...
If your application already owns OpenTelemetry setup, register Beacon's OpenAI instrumentation with your existing provider instead of calling `Observe.initialize()` twice.
