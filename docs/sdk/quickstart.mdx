...
description: "Install and initialize the Beacon TypeScript SDK"
---
...
Choose Beacon Managed hosted Observe or a customer-managed OTLP endpoint.

### Hosted Observe

Hosted Observe requires a Beacon Managed account. To get an `ASYMPTOTE_API_KEY`, [reach out for a demo](https://asymptotelabs.ai/contact), then initialize `Observe` as early as possible in your application startup:

```bash title="Set the Beacon Managed API key"
...
Use an explicit OTLP endpoint when traces should go to a local or customer-managed collector instead of Beacon Managed hosted Observe:
...
Explicit OTLP export does not require a Beacon Managed API key.
...
| 3 | Beacon Managed hosted `baseUrl` when `apiKey` or `ASYMPTOTE_API_KEY` is present |
