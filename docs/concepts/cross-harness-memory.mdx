...
4. Approve candidates into project memory, supplying the lesson text for a scored
   candidate as you do.
...
A trace becomes a candidate only when the task succeeded (`task_success` of at
least 0.50) and the mean of the three probabilities is at least 0.60. The
success answer is a precondition rather than one vote in the mean, so a failed
task cannot be promoted on the strength of the other two answers.
...
you run the command from another, and a linked worktree counts as its main checkout.
Commands that list or look up records scope to the current working directory. Use
`--project <path>` to override either, for example when a client needs to retrieve
...
`beacon memory list|show` reads the same approved records from the CLI.
...
The workflow itself ships as installable Agent Skills, so an agent in any harness can
drive it:

- `beacon-memory-recall` retrieves approved memory before a task.
- `beacon-memory-distill` runs the loop above. It dry-runs first, asks before the
  evaluation call, drafts each lesson from the source trace, and approves only what the
  user confirms.
- `beacon-memory-promote` installs approved memory as a project skill.

Install them with the harness's plugin marketplace, or with the skills CLI for any
...
# Codex, GitHub Copilot CLI, and Factory Droid read the same marketplace
codex plugin marketplace add asymptote-labs/agent-beacon

# Cursor, OpenCode, Amp, goose, Cline, and others
npx skills add asymptote-labs/agent-beacon
```

The skills live in `agent-skills/` in the Beacon repository. See its README for the full
install and publishing matrix.
