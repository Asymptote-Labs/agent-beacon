---
title: "Core Concepts"
description: "Core concepts for Beacon, Agent Beacon, and agent runtime telemetry"
---

Beacon observes AI agent runtime activity across supported local endpoints, CI jobs, cloud-agent sandboxes, and SDK-instrumented applications. Agent Beacon is the open-source foundation for local-first collection and customer-controlled forwarding. Beacon Managed builds on that foundation with hosted ingest, retention, search, detections, governance, and investigations.

Use this page as a quick reference for terms used across Beacon and Agent Beacon docs.

## Beacon

**Beacon** is the open-source, local-first telemetry system for supported AI agent activity. It collects events from local endpoints, CI jobs, and cloud-agent surfaces, normalizes them into consistent records, writes local JSONL, serves a local dashboard, and supports customer-managed forwarding. See [Open Source deployment](/deployment/open-source).

## Beacon Managed

**Beacon Managed** is the hosted Beacon operating model for centralized ingest, retention, search, detections, fleet visibility, identity mapping, approval visibility, SSO, RBAC, audit trails, and investigation workflows. See [Managed deployment](/deployment/managed).
...
The **Observe SDK** is the `@asymptote/sdk` TypeScript package for instrumenting cloud-hosted agent applications, services, workers, serverless functions, and agent SDK integrations. It can export to Beacon Managed hosted Observe or to a customer-managed OTLP collector. See [Beacon Observe SDK](/sdk).
...
The **dashboard** is a loopback-only local view over Beacon runtime logs and
adjacent local stores. It includes Agent Activity Sessions and session detail for
investigating timelines, Agent Activity for local summaries, Detections and
Findings for reviewing local threat-rule matches, Memory for read-only learning
state, Token Usage, and Agent Inventory without requiring a hosted Beacon account.
...
<Columns cols={2}>
  <Card title="Hosting Options" icon="cloud" href="/deployment/hosting-options">
    Compare Beacon, Managed, and Private Deployment paths.
  </Card>
  ...
  <Card title="Beacon Observe SDK" icon="code" href="/sdk">
