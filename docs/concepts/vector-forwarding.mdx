...
```mermaid
flowchart LR
  subgraph openSourcePath [Beacon]
  ...
  subgraph managedPath [Beacon Managed]
    managedSources["Endpoint, CI, cloud, and SDK telemetry"] -->|"managed ingest"| managedIngest["Beacon ingest"]
...
In **Beacon**, Agent Beacon is local-first. It configures supported agent harnesses, receives OTLP or hook telemetry, normalizes events, writes local runtime and inventory JSONL, and can generate forwarding content packs. Destination credentials, bucket policies, IAM roles, lifecycle rules, retention, encryption, and remote ingestion settings stay outside Beacon endpoint configuration.
...
[macOS System-Mode Object Storage Forwarding](/guides/macos-object-storage-system) for full setup and
rotation steps.
...
<Columns cols={2}>
  ...
  <Card title="Managed deployment" icon="cloud" href="/deployment/managed">
    Understand the hosted Beacon ingest, retention, search, detections, governance, and investigation model.
  </Card>
