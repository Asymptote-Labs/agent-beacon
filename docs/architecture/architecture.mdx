...
| [Browser](#browser) | Optional MV3 extension for Chromium browsers and beta Firefox, posting OTLP to the local collector | Persistent local runtime JSONL log |
...
Chat activity on supported sites is collected by an optional MV3 extension for Chromium browsers and beta Firefox. It reads the page's chat stream and posts OTLP logs to the local collector on `127.0.0.1:4318`. It writes no files and contacts no remote endpoint. Coverage is deliberately narrow: only the supported chat origins, never general browsing. See [Browser Extension](/runtimes/browser-extension).

### Agents in code

Agent applications, services, workers, and serverless functions are instrumented with the [Beacon Observe SDK](/sdk), which emits Beacon-compatible OpenTelemetry spans for model calls, tool calls, and agent steps. There is no endpoint agent in this path: the SDK exports to a customer-managed collector or to hosted Observe. See [SDK integrations](/sdk/integrations).
...
The local [dashboard](/concepts/core-concepts#dashboard) reads recent runtime logs
over a loopback-only service for rollout validation and investigation, with
session timelines, activity summaries, Detections and Findings, Token Usage,
Agent Inventory, and read-only Memory. Token attribution rolls up through the
dashboard token view and [`beacon token-usage`](/cli/token-usage). The dashboard
is read-only: it inspects local state and never mutates endpoint configuration,
telemetry, or memory review state.
