...
| Beacon managed ingest (opt-in) | In user mode, `beacon endpoint connect` uses the signed-in CLI account to authorize a separate per-device key; system mode uses browser approval. A Vector forwarder posts runtime and inventory JSONL over HTTPS to Beacon's ingest service; the device is revocable from the dashboard, effective within about a minute |

Beacon itself does not store Elastic cluster URLs or credentials, Datadog API keys or site configuration, Sumo Source URLs, tokens, collector configuration, or Rapid7 webhook URLs. Splunk HEC and Falcon LogScale HEC tokens are stored only in local collector configuration when those optional destinations are enabled. The Beacon device key is stored only in `asymptote/vector-secrets.json` (mode `0600`, inside a `0700` directory) under the endpoint base directory, read by Vector through its secret backend; `config.json` holds only the non-secret device and organization ids, and the key is never printed by any command.
...
`beacon endpoint uninstall` unloads the local Beacon service and removes managed endpoint files. Beacon removes files listed in its install manifest, including managed config, collector config, and service files. Unless `--keep-logs` is set, it also removes the configured runtime JSONL log. A connected endpoint's Beacon forwarder service and `asymptote/` directory (Vector config, buffer, enrollment record, device key) are removed too, except that `--keep-config` keeps the enrollment record and device key; `beacon endpoint disconnect` does only that part.
