...
| Beacon managed ingest (opt-in) | After a member of the organization approves the device in the browser (`beacon endpoint connect`), a Vector forwarder posts the runtime and inventory JSONL over HTTPS to Beacon's ingest service with a per-device key; revocable from the dashboard, effective within about a minute |
