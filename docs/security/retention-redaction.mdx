...
endpoint pipeline. Asymptote managed ingest, when a device has been connected,
forwards the runtime and inventory JSONL lines written after approval exactly as
stored locally, so the same retention and redaction settings govern what
reaches the Asymptote dashboard.
