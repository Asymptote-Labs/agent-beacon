...
|------|-----------------|
| Unintended hosted telemetry | Managed is preselected in interactive onboarding, but selection and account sign-in do not forward anything. Forwarding starts only after the separate connect command. Runtime forwarding starts at the end of the existing log; inventory starts at the beginning so the service receives the endpoint's current inventory baseline. The active privacy mode is recorded, and forwarding stops within a minute of revocation |
