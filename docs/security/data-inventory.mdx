...
| DeepSeek Harness | Managed native hooks plus local session-store sync | Live session, prompt, tool, command, file, MCP, subagent, and policy activity; committed assistant text/reasoning, failed tools, persisted results, and token usage from session backfill |
| GitHub Copilot CLI | Customer-managed OTLP HTTP plus local session-store sync | Live session, prompt, tool, and approval-like activity; committed assistant text/reasoning, complete tool results, commands, file activity, and token usage from session backfill |
| Kimi Code | Managed native hooks | Session, prompt, tool and failed-tool activity, commands with output and exit code, file reads/edits with diffs, MCP calls, subagents, compaction, and observed permission decisions; no token usage because hooks do not expose it |
...
`beacon memory` writes a local SQLite store, `memory.db`, next to the endpoint log
base directory. The store is separate from `runtime.jsonl` and keeps reviewed
learning artifacts scoped to the resolved project:

| Memory area | Data retained |
|-------------|---------------|
| Evaluations | Trace identity, project identity, bounded trace summary, Jev question results, evaluator model, usage metadata when returned, and evaluation timestamps |
| Candidates | Candidate title, body, kind, state, evidence pointers, source evaluation, project identity, and review transition metadata |
| Approved memories | Approved title, body, kind, project identity, provenance, and review metadata |

Approved memory can be read through local MCP tools (`search_memory`,
`get_memory`, and `get_memory_context`) and can be rendered into an Agent Skill
with `beacon memory skills install`. That skill write is an explicit CLI action
to `.agents/skills/<slug>/SKILL.md`; Beacon hooks and the dashboard never install
skills or mutate memory review state.

`beacon memory evaluations run` is the only command in this workflow that can call
Jev. Dry runs do not make a network request. When evaluation is enabled, Beacon
sends a bounded, redacted trace projection to the configured TypeSafe System One
endpoint or a compatible internal endpoint. Normal endpoint capture, MCP reads,
dashboard views, candidate review, and skill preview/install do not call Jev.
