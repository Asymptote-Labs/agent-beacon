...
| Qwen Code | Native hooks | Session, prompt, pre-tool, post-tool, failed tool, permission request, subagent, stop, session-end, command, and file telemetry with diffs; retained prompt text, tool inputs and results, command strings, file diffs, and the raw hook payload under `raw.qwen` |
