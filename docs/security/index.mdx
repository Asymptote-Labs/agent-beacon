...
Beacon is local-only by default. The endpoint agent does not require a Beacon-hosted account, remote policy fetch, or external network dependency during normal collection. Use [Beacon Managed](/deployment/managed) when you need centralized retention, search, governance, investigations, access control, or rollout support.
...
| Managed handoff | Whether the rollout now needs centralized visibility, investigations, SSO/RBAC, or private infrastructure. |
...
Open Source works well when your team wants local endpoint telemetry and controls the downstream destination. Consider [Beacon Managed](/deployment/managed) when you need:

- centralized ingest, retention, search, and detections
- fleet-wide visibility across endpoints, users, and teams
- identity mapping, approval visibility, and investigation workflows
