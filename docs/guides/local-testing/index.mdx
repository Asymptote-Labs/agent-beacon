...
<Steps>
  ...
  <Step title="Review the dashboard">
    Start `beacon endpoint dashboard --open` and confirm the validation event appears in local activity views. For real agent runs, open an Agent Activity Sessions row and inspect the session timeline.
  </Step>
...
<Columns cols={2}>
  ...
  <Card title="Test the Local Dashboard" icon="chart-line" href="/guides/local-testing/dashboard">
    Review activity sessions, event details, memory, findings, detections, token usage, and runtime inventory from the local dashboard.
  </Card>
