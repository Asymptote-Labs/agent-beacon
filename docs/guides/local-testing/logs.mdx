...
Session-file sync commands write through the same rotation. `beacon endpoint claude sync` stops before a rotation would discard its own output and leaves the rest pending; see [large backfills](/runtimes/claude-code#large-backfills-and-log-rotation).
