...
Model Context Protocol (MCP) lets local assistants connect to tools and data
sources through a standard interface. Beacon uses MCP to make local endpoint
activity searchable and approved project memory retrievable from an MCP client
without requiring a hosted Beacon account.
...
`beacon mcp` exposes compact activity tools over MCP so an assistant can search,
summarize, and fetch events from the local Beacon runtime log. It also exposes
approved-memory tools so future agents can reuse human-reviewed project knowledge
created through [`beacon memory`](/cli/memory).
...
| `search_memory` | Search approved Beacon project memory and return compact summaries. |
| `get_memory` | Fetch one approved memory item by ID. |
| `get_memory_context` | Return a small approved-memory context set for the current task. |

Memory tools are read-only. They do not evaluate traces, approve candidates, or
...
Transport: stdio
Runtime log: /Users/you/.beacon/endpoint/logs/runtime.jsonl
Runtime log source: user endpoint log
Runtime log check: ok
Sampled events: 1
Malformed lines: 0
Readable archives: 0
MCP tools: search_activity, summarize_activity, get_activity_event, list_activity_filters, search_memory, get_memory, get_memory_context
...
- Retrieve approved project memory as context for a new task without exposing
...
<Steps>
  ...
  <Step title="Connect an MCP client">
    Follow [Connect Cursor and Claude Code](#4-connect-cursor-and-claude-code) below, then ask the client to search recent Beacon activity, summarize activity from the last hour, or retrieve approved memory for the current project.
  </Step>
  ...
  <Step title="Compare with the dashboard">
    Open the dashboard and confirm MCP query results match the same event window,
    event IDs, and approved memory shown by the local views.
...
Restart Cursor or run **Developer: Reload Window**, then verify the server in
**Cursor Settings → MCP**. Beacon should connect with seven tools:
`search_activity`, `summarize_activity`, `get_activity_event`,
`list_activity_filters`, `search_memory`, `get_memory`, and
`get_memory_context`.
...
- Read-only retrieval of approved project memory through MCP.
