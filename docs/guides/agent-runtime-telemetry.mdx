...
In Agent Activity Sessions and Agent Activity, start with:
...
For a terminal-only investigation, open the local trace browser:

```bash title="Browse local traces"
beacon traces
```

Use `/` to search traces, `enter` to inspect one, and `1`–`4` to filter its
events by type. The browser is read-only and local; see
[`beacon endpoint traces`](/cli/endpoint-traces) for the JSON and scripting
interface.
