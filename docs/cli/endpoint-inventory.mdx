...
A system-mode job runs as root and inventories the active console user's home. When nobody is logged in it still writes the heartbeat, with `user_scope.mode` set to `no_console_user`, and no snapshot. The scheduled scan covers user-level configuration only; project-scoped files (`.cursor/mcp.json`, `.claude/settings.json` in a repository) are reported by `beacon endpoint inventory` run from that directory, not by the job.
