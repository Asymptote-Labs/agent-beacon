...
The server reads the configured Beacon runtime JSONL log and the approved-memory
store next to the endpoint log base directory. It exposes compact activity tools
plus read-only approved-memory tools to MCP clients. Use stdio for clients that
launch Beacon as a subprocess. Use HTTP only for local loopback testing or local
tools that connect to a running server.
