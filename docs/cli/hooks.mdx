...
| Flag | Description |
|------|-------------|
| `--harness <list>` | Comma-separated hook harnesses, such as `antigravity`, `claude`, `codex`, `cursor`, `devin-cli`, `devin-desktop`, `factory`, `grok`, `hermes`, `opencode`, `pi`, or `antigravity,claude,codex,cursor,devin-cli,devin-desktop,factory,grok,hermes,opencode,pi`. `devin` remains a legacy alias for `devin-cli`; `hermes-agent` maps to `hermes`; `pi-cli` maps to `pi`. Defaults to `cursor` |
...
Install the Pi extension at the user level:

```bash title="Install the Pi extension at the user level"
...
beacon endpoint hooks install --harness antigravity,claude,codex,cursor,devin-cli,devin-desktop,factory,grok,hermes,opencode,pi
```
...
```bash title="Check Pi extension status"
...
Remove the user-level Pi extension:

```bash title="Remove the user-level Pi extension"
