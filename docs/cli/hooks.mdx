...
<Note>
  Restart Antigravity CLI, Claude Code, Cline, Codex CLI, Cursor, Devin CLI, Devin Desktop, Factory Droid, Grok Build, Hermes Agent, OpenCode, or Qwen Code after installing or removing hooks so the updated hook configuration is picked up by new sessions.
</Note>
