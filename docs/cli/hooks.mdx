...
| DeepSeek Harness | `$DSH_HOME/beacon-endpoint-hooks.json` plus `$DSH_HOME/cordis.patch.yml` (default `~/.dsh`) | Not supported | `~/.beacon/endpoint/hooks/beacon-hooks` | `~/.beacon/endpoint/logs/runtime.jsonl` |
...
| Kimi Code | `$KIMI_CODE_HOME/config.toml` (default `~/.kimi-code/config.toml`) | Not supported | `~/.beacon/endpoint/hooks/beacon-hooks` | `~/.beacon/endpoint/logs/runtime.jsonl` |
...
| Cline | `~/.cline/plugins/beacon.ts` | `./.cline/plugins/beacon.ts` | `~/.beacon/endpoint/hooks/beacon-hooks` | `~/.beacon/endpoint/logs/runtime.jsonl` |
| Pi | `~/.pi/agent/extensions/beacon.ts` | `./.pi/extensions/beacon.ts` | `~/.beacon/endpoint/hooks/beacon-hooks` | `~/.beacon/endpoint/logs/runtime.jsonl` |
| Oh My Pi | `~/.omp/agent/extensions/beacon.ts` | `./.omp/extensions/beacon.ts` | `~/.beacon/endpoint/hooks/beacon-hooks` | `~/.beacon/endpoint/logs/runtime.jsonl` |
| Qwen Code | `~/.qwen/settings.json` | `./.qwen/settings.json` | `~/.beacon/endpoint/hooks/beacon-hooks` | `~/.beacon/endpoint/logs/runtime.jsonl` |
...
| Flag | Description |
|------|-------------|
| `--harness <list>` | Comma-separated hook harnesses, such as `antigravity`, `claude`, `cline`, `codex`, `cursor`, `devin-cli`, `devin-desktop`, `dsh`, `factory`, `grok`, `hermes`, `kimi`, `omp`, `openclaw`, `opencode`, `pi`, or `qwen`. `deepseek` and `deepseek-harness` map to `dsh`; `kimi-code`, `kimi_code`, and `moonshot` map to `kimi`; `devin` remains a legacy alias for `devin-cli`; `hermes-agent` maps to `hermes`; `qwen-code` maps to `qwen`; `pi-cli` and `pi_cli` map to `pi`; `oh-my-pi` and `ohmypi` map to `omp`; `openclaw-gateway` maps to `openclaw`. Defaults to `cursor` |
...
<Note>
  Restart Antigravity CLI, Claude Code, Cline, Codex CLI, Cursor, DeepSeek Harness, Devin CLI, Devin Desktop, Factory Droid, Grok Build, Hermes Agent, Kimi Code, or OpenCode after installing or removing hooks so the updated hook configuration is picked up by new sessions.
</Note>
...
Install Codex CLI session-context hooks at the user level:

```bash title="Install Codex CLI session-context hooks at the user level"
beacon endpoint hooks install --harness codex
```

A successful user-level install prints `Codex CLI hooks installed: /Users/you/.codex/hooks.json`.
...
Install Qwen Code hooks at the project level:

```bash title="Install Qwen Code hooks at the project level"
beacon endpoint hooks install --harness qwen --level project
```

Project-level Qwen Code hooks are only executed in a folder Qwen trusts, so a user-level install is the one that collects without further interaction.
...
Install Antigravity, Claude Code, Codex CLI, Cursor, Devin CLI, Devin Desktop, Factory, Grok Build, Hermes Agent, OpenCode, and Qwen Code hooks with the same runtime log:

```bash title="Install Antigravity, Claude Code, Codex CLI, Cursor, Devin CLI, Devin Desktop, Factory, Grok Build, Hermes Agent, OpenCode, and Qwen Code hooks with the same runtime log"
beacon endpoint hooks install --harness antigravity,claude,codex,cursor,devin-cli,devin-desktop,factory,grok,hermes,opencode,qwen
```
...
Check Qwen Code hook status:

```bash title="Check Qwen Code hook status"
...
Remove user-level Qwen Code hooks:

```bash title="Remove user-level Qwen Code hooks"
...
Qwen Code is a Gemini CLI fork without Gemini's OpenTelemetry export, so hooks are the only collection path for this runtime. `beacon endpoint install --harness qwen` installs these same hooks and writes `~/.qwen/settings.json`.
...
Beacon writes Codex CLI endpoint hooks to `~/.codex/hooks.json` for user-level installs or `./.codex/hooks.json` for project-level installs. `SessionStart` writes local OS-user/session context. This metadata-only hook does not replace the Codex OTLP runtime path for prompt, tool, or token events, and it no longer triggers inventory: that is the scheduled inventory job's work, in every mode, whether or not Codex runs.

If Codex session context is missing, check the selected level, confirm the hook file contains a command with `--platform codex`, and restart Codex CLI so new sessions pick up the updated hook configuration.
