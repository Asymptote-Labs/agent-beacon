...
Beacon writes Codex CLI endpoint hooks to `~/.codex/hooks.json` for user-level installs or `./.codex/hooks.json` for project-level installs. `SessionStart` writes local OS-user/session context and triggers inventory; `UserPromptSubmit` refreshes inventory. These metadata-only hooks do not replace the Codex OTLP runtime path for prompt, tool, or token events.
