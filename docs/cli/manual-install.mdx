...
`.zip` for Windows. Each archive contains three binaries: the `beacon` CLI, the `beacon-hooks` adapter that captures tool activity, and the matching `beacon-otelcol` collector. The Linux archives also carry `beacon-vector`, which is [Vector](https://vector.dev) for forwarding, named so it cannot shadow a `vector` already on your PATH. SHA-256 checksums are published alongside the release in `checksums.txt`.
...
[ -f beacon-vector ] && mv beacon-vector ~/.local/bin/   # Linux archives only
beacon version
```
...
verifies that file rather than whatever else the directory happens to hold.
