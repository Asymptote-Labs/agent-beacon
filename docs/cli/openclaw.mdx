...
<Note>
**This is the gateway half of OpenClaw support, not the whole of it.** These commands cover OpenClaw's own OpenTelemetry export, which carries gateway traces and metrics — and, unless `captureContent` is explicitly enabled, no prompt, tool, or file content at all.

What the **agent** did is collected by a separate, Beacon-managed plugin:

```bash
beacon endpoint hooks install --harness openclaw
```

Both paths report under the same `openclaw_gateway` harness and are wanted together. See [OpenClaw Gateway](/runtimes/openclaw-gateway) for the plugin, the config key token usage needs, and what is deliberately not collected.
</Note>
