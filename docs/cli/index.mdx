...
| [`beacon login`](/cli/account-auth) | Sign in through beacon.sh without enabling telemetry forwarding. |
| [`beacon logout`](/cli/account-auth#sign-out) | Revoke and remove the local Beacon account session. |
| [`beacon memory`](/cli/memory) | Evaluate traces, review candidates, and install approved project memory. |
...
| [`beacon traces`](/cli/endpoint-traces) | Open the interactive local trace browser; redirected I/O falls back to a bounded trace list. |
| [`beacon version`](/cli/version) | Display the installed Agent Beacon CLI version. |
| [`beacon whoami`](/cli/account-auth#inspect-the-account) | Show the current Beacon account and session expiry. |
...
| [`beacon cloud s3 setup`](/cli/cloud-s3-setup) | Create or print S3 setup commands for cloud-agent telemetry. |
...
| [`beacon endpoint status`](/cli/endpoint-status) | Inspect collector health, runtime log state, configured harnesses, diagnostics, forwarding readiness, and the Beacon managed-ingest connection. |
| [`beacon endpoint connect`](/cli/endpoint-connect) | Enroll this device with the signed-in CLI account and start the Vector forwarder; system mode uses browser approval. |
| [`beacon endpoint disconnect`](/cli/endpoint-connect) | Stop the Beacon forwarder and remove the local device key. |
...
| [`beacon endpoint update`](/cli/upgrade#self-updating-endpoint-packages) | Check for, enable, or apply signed endpoint package updates. |
| [`beacon endpoint config`](/cli/endpoint-config) | Inspect endpoint configuration and validate destination settings. |
| [`beacon endpoint onboarding`](/cli/endpoint-onboarding) | Show, reset, or resend the one-time onboarding record. |
...
| [`beacon endpoint traces`](/cli/endpoint-traces) | Index, list, search, and show local traces from Beacon runtime logs. |
...
| [`beacon endpoint asymptote`](/cli/asymptote) | Generate the Vector forwarding pack for Beacon managed ingest and write validation events. |
| [`beacon endpoint hooks`](/cli/hooks) | Manage supported hook-based endpoint telemetry. |
| [`beacon endpoint opencode`](/cli/endpoint-opencode) | Collect endpoint telemetry from OpenCode session records on this machine. |
| [`beacon endpoint pi`](/cli/endpoint-pi) | Collect endpoint telemetry from Pi session records on this machine. |
| [`beacon endpoint copilot`](/cli/endpoint-copilot) | Collect endpoint telemetry from GitHub Copilot CLI session records on this machine. |
| [`beacon endpoint dsh`](/cli/endpoint-dsh) | Collect endpoint telemetry from DeepSeek Harness session records on this machine. |
| [`beacon endpoint fx`](/cli/endpoint-fx) | Collect endpoint telemetry from fx session records on this machine. |
...
Generate the Vector forwarding pack that sends Beacon endpoint events to Beacon managed ingest, and write validation events.

| Command | Description |
|---------|-------------|
| [`beacon endpoint asymptote print-config`](/cli/asymptote#beacon-endpoint-asymptote-print-config) | Print the Vector forwarding config for the configured runtime log. |
| [`beacon endpoint asymptote install-pack`](/cli/asymptote#beacon-endpoint-asymptote-install-pack) | Write the Beacon forwarding pack to a directory. |
| [`beacon endpoint asymptote validate`](/cli/asymptote#beacon-endpoint-asymptote-validate) | Write and describe a known-good Beacon validation event. |
