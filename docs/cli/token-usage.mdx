...
- **Attribution depends on the metadata a runtime attaches.** Model, harness, repository, and run grouping work whenever those fields are present. For Codex, Beacon derives session-level token events from local Codex session JSONL when hook reconciliation is enabled, because Codex's OTLP token metric does not carry a conversation id.
...
| Codex CLI | input, output, cache read, reasoning | Not emitted by Codex | Yes, when local Codex session JSONL is available |

`cost_usd` remains empty for Codex because Codex does not emit a cost signal and Beacon does not derive cost locally. See the [Codex CLI](/runtimes/codex-cli#token-usage) and [Claude Code](/runtimes/claude-code) runtime pages for details.
...
| `--codex-source <auto\|runtime\|otlp\|both-debug>` | Select the Codex token source. `auto` prefers session-derived runtime events and suppresses OTLP duplicates. `both-debug` is diagnostic only |
| `--codex-sync` | Reconcile local Codex session files into the runtime log before reporting |
