...
| Cursor | input, output, cache (where Cursor records them) — via [`beacon token-usage sync-cursor`](/cli/token-usage-sync-cursor) | Not recorded by Cursor | Yes (conversation id) |
...
Cursor hook payloads carry no token counts, so Cursor rows appear after an
explicit [`beacon token-usage sync-cursor`](/cli/token-usage-sync-cursor) run, which
extracts runtime-recorded counts from Cursor's local state store. See the
[Cursor](/runtimes/cursor#token-usage) runtime page for details.
