...
| Runtime | Why |
|---------|-----|
| Cursor | Cursor's hook payloads carry no token counts |
...
| Qwen Code | The `Stop` payload carries session-level context counters, not per-call usage. Preserved under `raw.qwen` rather than normalized. See [Qwen Code](/runtimes/qwen-code) |
