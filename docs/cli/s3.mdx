...
<Columns cols={2}>
  <Card title="beacon endpoint s3 print-config" icon="file-lines" href="#beacon-endpoint-s3-print-config">
    Print a one-shot AWS CLI script that uploads the current logs to S3.
  </Card>
...
`beacon endpoint s3 print-config` prints a smoke-test script. The script uses the AWS CLI to copy the current runtime and inventory logs, once, to `<prefix>/runtime/smoke-tests/` and `<prefix>/inventory/smoke-tests/` in your bucket. Use it to check bucket access and IAM before you set up continuous forwarding. For the Vector configuration, use [`install-pack`](#beacon-endpoint-s3-install-pack).

```bash title="Print the smoke-test script"
beacon endpoint s3 print-config
```

### Examples

Print the script for the default per-user Beacon install:

```bash title="Print the script for the default per-user Beacon install"
beacon endpoint s3 print-config
```

Print the script for a system-mode MDM deployment:

```bash title="Print the script for a system-mode MDM deployment"
sudo /opt/beacon/bin/beacon endpoint s3 print-config --system
```

Print the script for a custom runtime log:

```bash title="Print the script for a custom runtime log"
...
`reasoning_output_tokens`, and `cost_usd`. The packaged macOS S3 forwarder that
MDM deployments use doesn't add these columns; its objects carry `gen_ai.usage`
only.
...
The validation command writes the local event only. The forwarder uploads in batches, so allow up to five minutes, then confirm delivery with AWS tooling. The event lands under `runtime/`:
...
<Columns cols={2}>
  <Card title="Beacon with Fleet and S3" icon="bucket" href="/guides/fleet-s3-mdm">
    Fleet software install, self-updates, and Vector forwarding to S3.
  </Card>
