...
The live hook paths for Claude Code, Cline and OpenCode record the link. Codex CLI prompts reach Beacon over OTLP and through `beacon endpoint codex sync`, not through a prompt hook, so a session continued in Codex is not linked yet.
