...
`--store-dir <runtime>=<path>` takes the location in the table: for Pi its sessions directory, as `beacon endpoint pi sync --sessions-dir` does, and for Prime Agent the agent directory holding both of its roots. For Factory Droid it is the sessions directory, as `beacon endpoint factory sync --sessions-dir` does. For fx it is its sessions directory, as `beacon endpoint fx sync --sessions-dir` does.
...
| Flag | Description |
|------|-------------|
| `--harness` | Only this runtime: `claude`, `codex`, `opencode`, `cline`, `pi`, `prime`, `droid` or `fx`. |
...
- **Session-store sync.** `beacon endpoint claude sync`, `codex sync`, `opencode sync`, `cline sync`, `pi sync`, `prime sync`, `factory sync` and `fx sync` record it when they read the prompt back from the runtime's store (`harness.collection_method: poll`).
