...
description: "Generate the Vector forwarding pack that sends Beacon endpoint events to Beacon's managed ingest service, and write validation events."
---

## Forwarding command

Use `beacon endpoint asymptote` to generate the content pack that forwards Beacon endpoint events to Beacon's managed ingest service. The pack keeps Beacon as a local JSONL producer: Vector tails the runtime and inventory logs and POSTs gzip-compressed NDJSON batches over HTTPS with a per-device key.

The hand-run pack uses Standard privacy and forwards locally sanitized lines
unchanged. Use [`beacon endpoint connect --privacy-mode metadata-only`](/cli/endpoint-connect#privacy-modes)
when Beacon should render the local metadata-only transforms for you.

This is the one destination that sends telemetry to a Beacon-run endpoint. It is
opt-in and revocable: nothing is forwarded until enrollment has issued a device
key and Vector has started. In user mode, `beacon endpoint connect` uses the
signed-in CLI account to authorize that key without another browser approval;
system mode uses browser approval. `beacon endpoint connect` performs enrollment
and runs the forwarder as a service; the commands on this page generate the same
files for running or inspecting the forwarder by hand.
...
The command writes the local event only. If the forwarder is running, the event reaches the ingest service within its batch window (60 seconds) and appears on the dashboard telemetry page a minute or so later. Search for the message `Beacon endpoint Beacon validation event`.
