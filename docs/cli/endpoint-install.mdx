...
| `--connect` | After a successful install, connect this endpoint to Asymptote managed ingest: opens the dashboard to approve the device and starts the Vector forwarder. Same as running [`beacon endpoint connect`](/cli/endpoint-connect) afterwards |

## Managed ingest offer

On a first interactive install where Vector is available, the [onboarding prompt](/cli/endpoint-onboarding) ends with one more question: whether to forward this machine's agent telemetry to Asymptote Managed now. The default is no. Answering yes runs the connect flow after the install completes; a failed connect is reported with the retry command and never fails the install. `--connect` is already the answer, so it skips the question. `BEACON_MANAGED_INGEST=0` suppresses the question and it is never shown non-interactively. A no is recorded at once and the question is not asked again. A yes is recorded only once the machine is connected, so if the install or the connection fails, the next interactive install asks again.
...
Qwen Code hooks merge into `~/.qwen/settings.json`. Qwen Code has no OpenTelemetry export to configure, so `beacon endpoint install --harness qwen` fails with a message pointing at `beacon endpoint hooks install` rather than appearing to succeed.
...
<Note>
  System-mode install (`--system`) is supported on macOS and on Linux distributions that use systemd. Omit `--system` for the default non-root local install. GitHub Copilot CLI and Factory Droid OTLP use launch-environment settings rather than Beacon-managed shell-profile edits.
</Note>
