...
The service is a launchd job on macOS and a systemd unit on Linux. Where neither is available, Beacon runs a supervised collector instead and says so. See [Linux install](/cli/endpoint-linux) for the Linux packages and the `--service` flag.
