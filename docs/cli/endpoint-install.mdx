...
| Answer | What happens |
|--------|--------------|
| Beacon Managed (default) | Shows what will be sent, asks for Standard or Metadata-only privacy, records both choices, and prints `beacon endpoint connect` as a separate next step. Nothing is forwarded during the wizard. |
| Local only | Explicitly opts out of managed forwarding. Nothing is sent anywhere. |

`--connect` presets Beacon Managed and attempts the connect flow after installation.
If connect fails, the completed local install still exits successfully and prints
`beacon endpoint connect` as the retry command. Automation should verify
