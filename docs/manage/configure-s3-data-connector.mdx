...
description: "Configure read-only S3 access for Beacon ingestion."
---

## Overview

Beacon can ingest customer-hosted event files from a private AWS S3 bucket or prefix. Grant Beacon's ingestion worker read-only access to the bucket that contains the event files.

Do not make the bucket public. Do not grant write or delete permissions.

At the end of setup, send Beacon the full IAM role ARN for the role you create.

## Customer steps

1. Send Beacon the S3 location details.
...
3. Configure the role trust policy.
   ...
   Use the IAM ARN Beacon gives you as the trusted principal. Replace `ASYMPTOTE_IAM_ARN` with that ARN.
...
6. Send Beacon the final onboarding details.
...
<Columns cols={2}>
  ...
  <Card title="Object Storage Forwarding" icon="bucket" href="/guides/macos-object-storage-system">
