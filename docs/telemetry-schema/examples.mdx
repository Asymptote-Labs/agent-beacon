...
```json
{
  "timestamp": "2026-05-11T22:21:00.418302511Z",
  "sequence": 12,
  "vendor": "beacon",
...
```json
{
  "timestamp": "2026-06-08T12:12:00.907441238Z",
  "vendor": "beacon",
  "product": "endpoint-agent",
  "schema_version": "1.0",
  "event": {
...
For Codex CLI, Beacon writes semantic session, prompt, approval, and tool-result events plus the single completed turn span carrying attributable token usage, while suppressing noisy startup, transport, metric, and duplicate span records by default. For GitHub Copilot CLI, Beacon maps OTLP spans into prompt, session, tool, and approval-like events. Beacon also filters generic process and runtime OTLP metrics, such as process CPU, memory, Node.js event loop, and V8 heap telemetry, plus Copilot CLI and OpenClaw operational metrics, out of the JSONL log by default. Use `--include-runtime-metrics` during endpoint install or repair only when those low-level metrics are required, and use `--include-codex-spans` only when every raw Codex span is needed for troubleshooting. The schema remains the same; these filters reduce low-signal records rather than changing event fields.
