---
title: "Asymptote Managed"
description: "Managed visibility, governance, and investigations for agent activity at scale"
tag: "Enterprise"
---

Asymptote Managed builds on the open-source Agent Beacon foundation for teams that need centralized operations, policy controls, and investigation workflows across a fleet.

Use this path when local JSONL and customer-managed forwarding are not enough on their own, or when agent adoption has moved beyond a small local pilot.

## What managed adds

Asymptote Managed includes the open-source capture and forwarding paths, then adds the managed services needed to operate agent security at company scale.

| Capability | What managed adds beyond open source |
| --- | --- |
| Managed ingest and retention | Centralized collection, retention, search, and detections for agent activity |
| Fleet-wide visibility | Inventory across endpoints, agent harnesses, users, and teams |
| Policy controls | Policy enforcement workflows, identity mapping, and approval visibility |
| Investigations | Timelines and case workflows for reviewing prompts, tools, file access, and agent actions |
| Access control | SSO, role-based access control, and audit trails |
| Operations support | Priority support, onboarding, and rollout guidance |

## Core managed workflows

### Centralize runtime visibility

Managed ingest turns endpoint and cloud-agent telemetry into searchable fleet-wide visibility. Security teams can review which agents are running, which tools they use, and what sensitive files or data are involved in agent activity.

Endpoints join with [`beacon endpoint connect`](/cli/endpoint-connect): the CLI opens the dashboard, a member of the organization approves the specific device, and a Vector forwarder ships the machine's runtime and inventory JSONL with a per-device key that the dashboard's Beacon Endpoints page can revoke at any time. The first interactive `beacon endpoint install` offers the same step. Enrollment is interactive per device today; headless enrollment for MDM fleets is a planned follow-up.

For provider-managed cloud agents, Managed avoids proof-of-concept patterns such
as placing storage service-account keys directly into cloud sandbox environment
variables. Asymptote can operate the secure ingest and retention layer for you,
or work with your team on a production-grade forwarding design in
customer-managed infrastructure.

### Govern agent activity

Policy, identity mapping, and approvals help teams connect runtime actions back to users and workflows. Use this when you need governance beyond a local audit log or SIEM-forwarded event stream.

### Investigate agent behavior

Investigation timelines and case workflows organize prompts, tool calls, file access, command execution, and related context so security teams can reconstruct what happened during an agent session.

### Support enterprise rollout

Managed deployments include rollout planning, onboarding, and support for teams deploying across endpoints and execution environments.

## Private deployment

For regulated teams that need dedicated infrastructure or stricter data boundaries, Asymptote supports private cloud or customer-controlled deployment options.

Private deployment can include:

- complete data isolation and residency options
- custom retention and forwarding architecture
- coverage across endpoints and execution environments
- dedicated deployment engineering and rollout planning

## Get access

Request access or discuss deployment requirements at [asymptotelabs.ai/contact](https://asymptotelabs.ai/contact). For plan details, see [Asymptote pricing](https://asymptotelabs.ai/pricing).

## Related

<Columns cols={2}>
  <Card title="Hosting Options" icon="cloud" href="/deployment/hosting-options">
    Compare Open Source, Managed, and Private Deployment paths.
  </Card>
  <Card title="Asymptote Open Source" icon="download" href="/deployment/open-source">
    Install Beacon locally and configure endpoint telemetry.
  </Card>
  <Card title="Security & IT Rollout Guide" icon="building-shield" href="/security">
    Plan endpoint inventory, MDM rollout, validation, retention, and forwarding.
  </Card>
  <Card title="Asymptote Observe SDK" icon="code" href="/sdk/quickstart">
    Instrument cloud-hosted agent applications with Asymptote telemetry.
  </Card>
</Columns>
