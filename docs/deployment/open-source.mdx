...
<Note>
  Setting up the endpoint differs by operating system. This section describes the per-user install,
  which works on both. For the platform detail — packages, service management, and fleet deployment —
  see [macOS](/platforms/macos) or [Linux](/platforms/linux).
</Note>
