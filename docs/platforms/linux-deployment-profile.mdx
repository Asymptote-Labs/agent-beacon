...
Four static executables, no shared libraries, no minimum glibc version. They run the same on a minimal or older userland as on a full one.
...
| `beacon` | CLI | No |
| `vector` | Forwarder ([Vector](https://vector.dev) 0.56) | Only if you set up a destination that uses it |
...
| Disk, log | ≤60 MB | 10 MiB × 5 rotations, oldest discarded |
| Disk, package | ~220 MB installed, ~86 MB download (amd64) | About 150 MB of it is Vector |
...
| `beacon.sh` | Interactive install only: browser sign-in, and the device enrollment that follows when Beacon Managed is confirmed | `BEACON_MANAGED_INGEST=0` hides the managed answer; `BEACON_ONBOARDING=0` skips the wizard entirely |
| Your ingest host | Continuously, only on an endpoint connected to Beacon Managed | `beacon endpoint disconnect`, or never connecting |
...
A forwarding destination you set up yourself, such as Splunk HEC, or Microsoft Sentinel through Vector, connects to that destination's hosts. Nothing does until you configure one.
...
On RPM systems, `dnf remove` keeps `/etc/beacon/endpoint` and `/var/log/beacon-agent`, and there is no purge. To remove the data too, run `sudo beacon endpoint uninstall --system` before `sudo dnf remove beacon`; afterwards the `beacon` binary is gone.
