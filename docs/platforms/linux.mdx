...
```bash title="Debian, Ubuntu, Fedora, RHEL, Rocky, Alma"
curl -fsSL https://github.com/asymptote-labs/agent-beacon/releases/latest/download/install.sh | bash
```

The [installer](https://github.com/asymptote-labs/agent-beacon/blob/main/packaging/linux/install.sh) downloads the latest `.deb` or `.rpm` for your architecture into a temporary directory, checks it against the release checksums, installs it with `apt-get` or `dnf`, and cleans up. Set `BEACON_VERSION=<version>` to install a specific release.

To run the same steps yourself, use the block for your distribution:
...
<Note>
APT reads a local package as its unprivileged `_apt` user. If the `.deb` sits somewhere `_apt` cannot read, such as your home directory, the install still succeeds, but APT ends with `Download is performed unsandboxed as root` and `pkgAcquire::Run (13: Permission denied)`. That is a warning, not a failure, and there is nothing to repair; `beacon version` confirms the install. The installer and the Debian block above download into a readable directory under `/tmp` so the warning does not appear.
</Note>
...
tar -xzf "${ARCHIVE}"
mkdir -p ~/.local/bin && mv beacon beacon-hooks beacon-otelcol beacon-vector ~/.local/bin/
...
The archive contains four binaries: the `beacon` CLI, the `beacon-hooks` adapter that captures tool activity, the `beacon-otelcol` collector, and `beacon-vector`, which is [Vector](https://vector.dev) for forwarding (see [Forwarding](#forwarding)). The first three are needed. `beacon-vector` only runs if you set up a destination that uses it, and it is named that so it cannot shadow a `vector` you already have on PATH. All four are statically linked with no shared-library dependencies.
...
Inventory heartbeat: scheduled every 6h0m0s; job loaded=true running=true; last emitted never
```

`running=true` and `telemetry=enabled` are the lines that matter. Then run your agent as usual (see [Using it](#using-it)) and its events land in the runtime log. `Beacon Managed: not connected` is expected, because a package install keeps everything on this machine.

### If something looks wrong

`beacon endpoint doctor --system` checks the whole chain (unit, collector, config, log permissions, and each harness's settings) and prints the command that fixes anything it finds. What matters is `0 failure(s)`. Before you have run an agent it warns `harness_observed`, which clears the first time you do.
...
To test the pipeline without running an agent, write a synthetic event with `sudo beacon endpoint test-event --system`. Keep the `--system`: without it the command writes to the user-mode log under `~/.beacon` and reports success against the wrong file.
...
A new install is local-only. Nothing leaves the machine until you set up a destination, and each one reads the same local log.

| Destination | What ships the events |
|---|---|
| [Splunk](/log-forwarding/splunk) | The collector, over HEC |
| [Elastic](/log-forwarding/elastic), [Datadog](/log-forwarding/datadog), [Wazuh](/log-forwarding/wazuh) | That product's own agent, tailing the log |
| [Microsoft Sentinel](/log-forwarding/microsoft-sentinel) | Vector, through the Logs Ingestion API. Or Azure Monitor Agent, which outside Azure needs Azure Arc first |
| [S3](/log-forwarding#aws-s3), [GCS](/log-forwarding#google-cloud-storage), [CloudWatch](/log-forwarding/cloudwatch), [Sumo Logic](/log-forwarding/sumo), [Rapid7](/log-forwarding/rapid7), [CrowdStrike Falcon](/log-forwarding/falcon) | Vector |
| [Beacon Managed](/log-forwarding/asymptote) | Vector, set up by `beacon endpoint connect` |

Vector comes with Beacon. The `.deb` and `.rpm` install it at `/opt/beacon/bin/vector`, and the tarball carries it as `beacon-vector`. Neither is on PATH by default, and neither runs until a destination needs it. `beacon endpoint connect` finds it on its own. For the other Vector destinations, run the generated `vector.toml` with `/opt/beacon/bin/vector`, or with your own Vector 0.50 or newer.

## What the install does

| Step | Result |
|---|---|
| Unpack | `beacon`, `beacon-hooks`, `beacon-otelcol` and `vector` under `/opt/beacon/bin` |
...
On an interactive terminal this opens the setup wizard, which asks you to sign in. Sign-in finishes in a browser on this machine, so over SSH or with no display the wizard says so straight away and offers to finish without an account, or to show the sign-in URL with the `ssh -L` command for opening it on your own computer. See [Onboarding](/cli/endpoint-onboarding). `BEACON_ONBOARDING=0` skips the wizard, and package installs never show it.
...
```bash title="Remove everything (Debian, Ubuntu)"
sudo apt purge beacon
```

RPM has no purge. Run Beacon's own uninstall first, while the binary is still there, then remove the package:

```bash title="Remove everything (RHEL, Rocky, Fedora)"
sudo beacon endpoint uninstall --system
sudo dnf remove beacon
```

`beacon endpoint uninstall --system` on its own also removes a non-package install. `--keep-logs` and `--keep-config` are available if you want to keep either.
