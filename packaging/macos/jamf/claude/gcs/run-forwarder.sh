...
CONFIG_PATH="${BEACON_GCS_VECTOR_CONFIG:-/Library/Application Support/Beacon/Forwarders/gcs-vector.toml}"
ENV_PATH="${BEACON_GCS_VECTOR_ENV:-/Library/Application Support/Beacon/Forwarders/gcs-vector.env}"
...
if [ -z "${GOOGLE_APPLICATION_CREDENTIALS:-}" ] || [ ! -r "$GOOGLE_APPLICATION_CREDENTIALS" ]; then
  echo "GOOGLE_APPLICATION_CREDENTIALS is missing or unreadable" >&2
  exit 1
fi
