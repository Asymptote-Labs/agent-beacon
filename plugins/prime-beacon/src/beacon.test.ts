...
const senderKey = Symbol.for("beacon.prime.testSender")
...
// A stand-in for Prime Agent's ExtensionAPI that records subscriptions and lets a test fire one.
function fakePi() {
...
describe("beacon prime agent extension", () => {
  // These strings are the contract between this extension and the prime-event mapper in the hook
  ...
  // Prime Agent publishes many more events than these, most of them provider-request, refinement
  // and TUI internals. Subscribing to one would put Beacon in the path of every streaming token
  // update.
  ...
  // Prime Agent keeps identity on the handler context behind accessor functions, not on the event,
  // so the envelope has to carry it or every event loses the field that groups a run.
  ...
  // Re-read per event rather than captured at load: `/new`, a resume, a fork and `/tree` all
  ...
  // Prime Agent's session events carry an AbortSignal and its message events carry live
  // AgentMessage objects, so a payload JSON.stringify would reject is the normal case, not an edge
  // case. Its `ipython` results go further: they carry kernel attachments and streamed diffs.
  test("serializes an event containing a cycle", async () => {
    ...
    await pi.fire({ type: "tool_result", toolName: "ipython", details: selfReferential }, context())
  ...
  // Every handler must resolve to undefined. Prime Agent reads a returned object as a request to
  // change behavior -- `{ block: true }` on tool_call, `{ action: "transform" }` on input,
  // `{ operations }` on user_bash -- so a handler that returned anything would turn this observer
  // into an enforcer.
  test("handlers return nothing so Prime Agent never reads a directive", async () => {
    captureSends()
    const pi = fakePi()
    createBeaconExtension().register(pi.api)
  ...
  // Event fields win over the lifted identity fields on a key collision, so a value Prime Agent
  // reported is never overwritten by one Beacon derived.
  ...
  // Prime Agent's only default tool. The mapper reads the cell's code off `input.code` and the
  // outcome off `details`, so an envelope that dropped either would leave the runtime's entire
  // agent surface unrecorded.
  test("forwards an ipython cell with its code and kernel details", async () => {
    const sent = captureSends()
    const pi = fakePi()
    createBeaconExtension().register(pi.api)
    ...
    await pi.fire(
      {
        ...
        toolName: "ipython",
        toolCallId: "call-1",
        input: { code: "open('a.py','w').write('x')" },
        details: {
          status: "ok",
          durationMs: 12,
          stdout: "done\n",
          diffs: [{ path: "/repo/a.py", oldStr: "x = 1", newStr: "x = 2" }],
        },
      },
      context(),
    )
    ...
    expect((sent[0].input as Record<string, unknown>).code).toBe("open('a.py','w').write('x')")
    expect(details.status).toBe("ok")
    expect((details.diffs as unknown[]).length).toBe(1)
  })

  // Prime Agent exposes no `mode` string the way Pi and Oh My Pi do. `hasUI` is the fact those were
  // read for: an unattended print or JSON run is not a run somebody was watching.
  test("records whether the session had a user-facing surface", async () => {
    const sent = captureSends()
    const pi = fakePi()
    createBeaconExtension().register(pi.api)

    await pi.fire({ type: "input", text: "hello" }, context({ hasUI: false }))
    await pi.fire({ type: "input", text: "hello" }, context())

    expect(sent[0].primeHasUi).toBe(false)
    // Absent rather than defaulted: a host that did not report it should not be recorded as one
    // that reported "no".
    expect(sent[1].primeHasUi).toBeUndefined()
  })

  // The runtime publishes no approval event, and Beacon refuses to synthesize one from a blocking
  // tool handler. This pins the absence from the extension's side: the Go mapper cannot invent an
  // approval for a runtime that never sends one.
  test("subscribes to no approval event", () => {
    const pi = fakePi()
    createBeaconExtension().register(pi.api)

    for (const name of [...pi.handlers.keys()]) {
      expect(name).not.toContain("approval")
    }
  })
})
