// Package selfupdate implements Beacon's built-in self-updater: discovering a
// newer signed/notarized/stapled .pkg from a release manifest, verifying it, and
// applying it via the macOS installer. Discovery (the read-only check) is
// cross-platform; applying updates is macOS-only and operates on the system
// /opt/beacon package install.
package selfupdate
