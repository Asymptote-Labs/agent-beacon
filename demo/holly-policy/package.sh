...
#   demo/holly-policy/dist/pkg/{beacon-policy,beacon-policy.sha256,install.sh,uninstall.sh}
set -euo pipefail
cd "$(dirname "$0")/../.."
VERSION="holly-policy-$(git rev-parse --short=8 HEAD)"
OUT=demo/holly-policy/dist/pkg
...
cp demo/holly-policy/install.sh demo/holly-policy/uninstall.sh "$OUT/"
